                    ...
                    if book_resp.status == 200:
                        ...
                        if book_data.get("code") == "0" and book_data.get("data"):
                            ...
                            if asks and bids:
                                ...
                                max_buy_price = best_ask * 1.001  # ✅ ИСПРАВЛЕНО: 0.1% выше best_ask (было 50% спреда)
                                min_sell_price = best_bid * 0.999  # ✅ ИСПРАВЛЕНО: 0.1% ниже best_bid (было 50% спреда)
                                ...
                                if max_buy_price > best_ask * 1.001:
                                    max_buy_price = best_ask * 1.001  # ✅ ИСПРАВЛЕНО: Не более 0.1% (было 1%)
                                if min_sell_price < best_bid * 0.999:
                                    min_sell_price = best_bid * 0.999  # ✅ ИСПРАВЛЕНО: Не более 0.1% (было 1%)
