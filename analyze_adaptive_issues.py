...
class AdaptiveIssueAnalyzer:
    ...
    def analyze_duration_issues(self):
        ...
        print(f"\n📊 СТАТИСТИКА:")
        print(f"   Отрицательных duration: {negative_count} ({negative_count/len(self.trades)*100:.1f}%)")
        print(f"   Нулевых duration: {zero_count} ({zero_count/len(self.trades)*100:.1f}%)")
        ...
        for symbol in symbols:
            ...
            negative = len([i for i in symbol_issues if i["type"] == "negative_duration"])
        ...
        for reason in reasons:
            ...
            negative = len([i for i in reason_issues if i["type"] == "negative_duration"])
    ...
    def analyze_max_holding_issues(self):
        ...
        max_holding_trades = [t for t in self.trades if t.get("reason") == "max_holding_exceeded"]
        ...
        for symbol in symbols:
            ...
            print(f"   {symbol}: {len(symbol_trades)} сделок, PnL=${total_pnl:+.2f}, средний=${avg_pnl:+.2f}")
        ...
        if durations:
            ...
            print(f"   Средний duration: {avg_duration:.0f}s ({avg_duration/60:.1f} мин)")
    ...
    def analyze_entry_time_issues(self):
        ...
        negative_duration_trades = [t for t in self.trades if float(t.get("duration_sec", 0)) < 0]
...
def main():
    csv_path = Path("logs/futures/archived/logs_2025-12-06_15-58-40/trades_2025-12-04.csv")
