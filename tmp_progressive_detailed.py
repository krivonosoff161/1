...
def calculate_progressive_size(balance, profile_name):
    ...
    if profile_name == "micro":
        if balance <= profile["min_balance"]:
            ...
            progress = (balance - profile["min_balance"]) / (profile["threshold"] - profile["min_balance"])
            return profile["size_at_min"] + (profile["size_at_max"] - profile["size_at_min"]) * progress
        ...
        if balance <= profile["min_balance"]:
            ...
            progress = (balance - profile["min_balance"]) / (profile["max_balance"] - profile["min_balance"])
            return profile["size_at_min"] + (profile["size_at_max"] - profile["size_at_min"]) * progress

# Тестовые балансы
test_balances = [200, 300, 400, 500, 600, 800, 1018, 1200, 1500, 1800, 2000, 2500, 3000, 4000, 5000, 7500, 10000]
...
for balance in test_balances:
    ...
    print(f"${balance:<9} {profile_name:<10} ${position_size:<11.2f} ${total_margin:<14.2f} {usage_percent:>6.2f}%        ${available:<14.2f} ${total_notional:<14.2f} ${monthly_profit:>7.2f}")
...
for symbol, multiplier in symbol_multipliers.items():
    ...
    print(f"{symbol:<15} {multiplier:<12.2f} ${symbol_size:<17.2f} ${symbol_margin:<11.2f} {symbol_percent:>6.2f}%")
...
for symbol, multiplier in symbol_multipliers.items():
    ...
    print(f"{symbol:<15} ${symbol_size:<11.2f} ${symbol_notional:<14.2f} ${symbol_margin:<11.2f} {symbol_percent:>6.2f}%")

print(f"\n📊 ИТОГО (все 5 позиций открыты):")
print(f"   Общий номинальный объем: ${total_notional_all:.2f} ({(total_notional_all/balance)*100:.2f}% баланса)")
print(f"   Общая маржа: ${total_margin_all:.2f} ({(total_margin_all/balance)*100:.2f}% баланса)")
print(f"   Запас: ${balance - total_margin_all:.2f} ({((balance - total_margin_all)/balance)*100:.2f}% баланса)")
...
for loss_percent in [0.05, 0.10, 0.15, 0.20]:
    ...
    print(f"   При убытке -{loss_percent*100:.0f}%: убыток ${loss:.2f}, остаток ${remaining:.2f} ({remaining_percent:.1f}%) {status}")
...
print(f"   Прирост: ${target_balance - initial_balance} ({((target_balance - initial_balance)/initial_balance)*100:.1f}%)")
...
for day in range(0, days + 1):
    ...
    print(f"{day:<6} ${current_balance:<9.2f} {profile_name:<10} ${position_size:<11.2f} ${total_margin:<14.2f} {usage_percent:>6.2f}%        ${monthly_profit:>7.2f}")
...
print(f"      Общая маржа: ${total_margin:.2f} ({(total_margin/balance)*100:.2f}% баланса)")
print(f"      Запас: ${balance - total_margin:.2f} ({((balance - total_margin)/balance)*100:.2f}% баланса)")
...
print("\n✅ ПРИБЫЛЬНОСТЬ:")
profit_total = sum(((base_size * m * 0.5) / 100) - ((base_size * m * 0.0009) * 2) for m in symbol_multipliers.values()) * 10 * 30
print(f"   Месячная прибыль: ~${profit_total:.2f} ({(profit_total/balance)*100:.1f}% ROI)")
