                                        ...
                                        if replace_with_market:
                                            ...
                                            try:
                                                ...
                                                if size_in_contracts > 0 and side in [
                                                    ...
                                                    logger.info(
                                                        f"📈 Размещаем market ордер вместо зависшего лимитного "
                                                        f"(таймаут): {symbol} {side} {size_in_coins:.6f} "
                                                        f"(было {size_in_contracts:.6f} контрактов, висел {wait_time:.0f} сек)"
                                                    )
                                                    ...
                                                    if result.get("success"):
                                                        logger.info(
                                                            f"✅ Market ордер размещен вместо лимитного (таймаут): "
                                                            f"{result.get('order_id')}"
                                                        )
                                                    else:
                                                        logger.error(
                                                            f"❌ Не удалось разместить market ордер для {symbol}: "
                                                            f"{result.get('error', 'unknown error')}"
                                                        )
                                            except (ValueError, TypeError) as e:
                                                logger.debug(
                                                    f"Ошибка парсинга размера ордера {order_id} при замене на market: {e}"
                                                )
