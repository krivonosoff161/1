...
class OrderExecutorConfig(BaseModel):
    ...
    stp_mode: str = Field(default="CancelMaker", description="Self-Trade Prevention mode")
    use_ioc_fallback: bool = Field(default=True)
    use_oso_oco: bool = Field(default=True)
    oso_primary_type: str = Field(default="limit", description="Primary order type for OSO-OCO")
