...
class ExitAnalyzerMonitor:
    ...
    def parse_log_line(self, line: str) -> Optional[Dict]:
        ...
        if match:
            symbol, entry_price, current_price, side, pnl_pct, entry_time = match.groups()
            return {
                'symbol': symbol,
    ...
    def calculate_expected_pnl(self, entry_price: float, current_price: float, side: str) -> float:
    ...
    def analyze(self) -> Dict:
        ...
        for line in new_lines:
            ...
            # Ищем закрытия
            if 'ExitAnalyzer: Закрываем' in line or 'ExitAnalyzer решение.*close' in line:
    ...
    def print_report(self, results: Dict):
        """Выводит отчет"""
        print(f"\n{'='*60}")
        print(f"📊 ОТЧЕТ ExitAnalyzer Monitor - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"{'='*60}\n")
        ...
        if positions:
            ...
            for symbol, data in positions.items():
                print(f"   {symbol}:")
                print(f"      Entry: ${data['entry_price']:.2f}, Current: ${data['current_price']:.2f}")
...
def main():
    ...
    # Путь к лог-файлу
    log_file = Path(__file__).parent.parent / "logs" / "futures" / "futures_main_2025-11-30.log"
