...
for variant_name, position_size in position_size_options.items():
    ...
    print(
        f"   Маржа: ${margin_per_position:.2f} ({(margin_per_position/balance)*100:.2f}% баланса)"
    )

    print(f"\n📊 МАКСИМУМ ПОЗИЦИЙ ({max_positions}):")
    print(
        f"   Номинальный объем: ${total_notional:.2f} ({notional_percent:.2f}% баланса)"
    )
    ...
    print(f"\n⚠️ РИСК ЛИКВИДАЦИИ:")
    print(
        f"   Падение цены для ликвидации: ~{liquidation_price_drop:.1f}% (теоретически)"
    )
    ...
    for loss_percent in loss_scenarios:
        ...
        print(
            f"      Убыток: ${loss_amount:.2f} ({(loss_amount/balance)*100:.1f}% баланса)"
        )
        print(
            f"      Остаток: ${remaining_balance:.2f} ({remaining_percent:.1f}%) {status}"
        )
    ...
    print(
        f"   Прибыль на сделку: ${profit_net_per_trade:.2f} (валовая ${profit_gross_per_trade:.2f}, комиссия ${commission_per_trade:.2f})"
    )
    ...
    print(
        f"      Месячная прибыль: ${monthly_profit_5_pos:.2f} ({roi_monthly_5_pos:.1f}% ROI)"
    )
...
for balance in balances:
    ...
    print(
        f"${balance:<9} {profile_name:<10} ${position_size:<9.2f} ${total_margin:<11.2f} {usage_percent:>6.2f}%        ${available:<11.2f} ${monthly_profit:>7.2f}      {roi:>5.1f}%"
    )
...
for scenario_name, change_percent, is_profit in scenarios:
    ...
    print(
        f"      Новый баланс: ${new_balance:.2f} ({new_balance_percent:.1f}%) {status}"
    )
...
loss_per_trade = (
    position_size * 0.015
) * max_positions  # Предполагаем все позиции открыты
...
print(
    f"   Новый баланс: ${new_balance_sequence:.2f} ({(new_balance_sequence/balance)*100:.1f}%)"
)
...
for num_positions in [1, 2, 3, 4, 5]:
    ...
    for loss_percent in [0.10, 0.15, 0.20]:
        ...
        print(
            f"   При убытке -{loss_percent*100:.0f}%: убыток ${loss:.2f}, остаток ${remaining:.2f} ({remaining_percent:.1f}%) {safety}"
        )
...
recommendations = [
    ...
    {"name": "Умеренный", "size": 120.0, "description": "Баланс риск/прибыль"},
    {"name": "Рекомендуемый", "size": 150.0, "description": "Оптимальный баланс"},
    {
        "name": "Агрессивный",
        "size": 200.0,
...
for rec in recommendations:
    ...
    print(
        f"{rec['name']:<15} ${size:<9.2f} ${margin:<14.2f} {usage:>6.2f}%        ${available:<14.2f} ${monthly_profit:>7.2f}      ${loss_10_percent:>7.2f} (остаток ${remaining_after_loss:.0f})"
    )
