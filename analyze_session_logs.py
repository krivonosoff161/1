...
def analyze_logs(logs_dir):
    ...
    patterns = {
        ...
        "close_success": re.compile(r"✅ Позиция (\S+) успешно закрыта по причине: (\S+).*?Net PnL: \$([+-]?\d+\.?\d*)"),
        ...
        try:
            ...
            for line in lines:
                ...
                if match:
                    ...
                    current_close = {"symbol": symbol, "side": side, "source": "position_manager"}
                    continue
                        ...
                        if m:
                            ...
                            if "symbol" in current_close:
                                ...
                                if "net_pnl" in current_close:
                                    ...
                                    results["symbols_stats"][symbol]["pnl"] += current_close["net_pnl"]
...
def generate_report(results):
    ...
    for symbol, stats in sorted(results["symbols_stats"].items()):
        report.append(f"{symbol:<15} {stats['opens']:<10} {stats['closes']:<10} {stats['pnl']:+.4f}")
    report.append("-" * 60)
    report.append("")
    ...
    for i, close in enumerate(closes_with_pnl[:100], 1):  # Первые 100
        ...
        report.append(f"{i:3d}. {symbol:<12} {side:<6} | Entry: ${entry:.4f} → Exit: ${exit_p:.4f} | "
