    ...
    def _get_scaling_config(
        ...
        if not scaling_config:
            ...
            default_config = {
                ...
                "ladder": [1.0, 0.5, 0.3, 0.2],  # ✅ ИСПРАВЛЕНИЕ: Оптимизированная лестница (сумма 2.0x вместо 3.45x)
            }
            logger.info(
                ...
                if profile_config:
                    config_dict = {
                        ...
                        "ladder": getattr(
                            profile_config,
                            "ladder",
                            [1.0, 0.5, 0.3, 0.2],  # ✅ ИСПРАВЛЕНИЕ (25.12.2025): Оптимизированная лестница
                        ),
                        "max_additions": min(getattr(profile_config, "max_additions", 4), 4),  # ✅ ИСПРАВЛЕНИЕ: Ограничение до 4
                    }
                    logger.debug(
        ...
        # Базовый конфиг
        config_dict = {
            "max_additions": min(getattr(scaling_config, "max_additions", 4), 4),  # ✅ ИСПРАВЛЕНИЕ (25.12.2025): Ограничение до 4
            "min_interval_seconds": getattr(scaling_config, "min_interval_seconds", 30),
            "max_loss_for_addition": getattr(
                scaling_config, "max_loss_for_addition", -3.0
            ),
            "ladder": getattr(
                scaling_config, "ladder", [1.0, 0.5, 0.3, 0.2]  # ✅ ИСПРАВЛЕНИЕ (25.12.2025): Оптимизированная лестница
            ),
        }
        return config_dict
