...
async def test_short_position_lifecycle():
    ...
    print(f"   BTC: {after_sell_btc:.8f} (было {initial_btc:.8f}, diff: {after_sell_btc - initial_btc:.8f})")
    print(f"   USDT: ${after_sell_usdt:.2f} (было ${initial_usdt:.2f}, diff: ${after_sell_usdt - initial_usdt:.2f})")
    ...
    try:
        ...
        if oco_status.get("data"):
            for order in oco_status["data"][:3]:  # Первые 3
                print(f"   OCO {order.get('algoId')}: {order.get('state')} | {order.get('instId')}")
    ...
    print(f"   USDT: ${after_wait_usdt:.2f} (diff: ${after_wait_usdt - after_sell_usdt:.2f})")
    ...
    if abs(after_wait_btc - after_sell_btc) > 0.00000001:
        ...
        print(f"   BTC: {final_btc:.8f} (начало: {initial_btc:.8f}, diff: {final_btc - initial_btc:.8f})")
        print(f"   USDT: ${final_usdt:.2f} (начало: ${initial_usdt:.2f}, diff: ${final_usdt - initial_usdt:.2f})")
...
async def test_long_position_lifecycle():
    ...
    print(f"   ETH: {after_buy_eth:.8f} (было {initial_eth:.8f}, купили: {actual_bought:.8f})")
    print(f"   USDT: ${after_buy_usdt:.2f} (было ${initial_usdt:.2f}, diff: ${after_buy_usdt - initial_usdt:.2f})")
    ...
    print(f"   USDT: ${after_wait_usdt:.2f} (diff: ${after_wait_usdt - after_buy_usdt:.2f})")
    ...
    if abs(after_wait_eth - after_buy_eth) > 0.00000001:
        ...
        final_usdt = await client.get_balance("USDT")
        print(f"   ETH: {final_eth:.8f} (начало: {initial_eth:.8f}, diff: {final_eth - initial_eth:.8f})")
        print(f"   USDT: ${final_usdt:.2f} (начало: ${initial_usdt:.2f}, diff: ${final_usdt - initial_usdt:.2f})")
