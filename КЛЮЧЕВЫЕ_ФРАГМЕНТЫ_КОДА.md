# 🔍 Ключевые фрагменты кода для анализа

## 1. trading_control_center.py (строка ~258)
...
import os
import time
from datetime import datetime as dt  # ✅ Используется как dt
...
**Примечание:** В этом файле используется `dt` вместо `datetime`, поэтому ошибок datetime здесь нет.

---

## 2. position_manager.py (строка ~5797: close_position_manually)

```python
# src/strategies/scalping/futures/position_manager.py

# Строки 15-16: Импорты
from datetime import datetime, timezone

# Строки 5761-5808: close_position_manually
    ...
    async with self._close_lock:
        try:
            ...
            if entry_time:
                ...
                exit_time = datetime.now(timezone.utc)  # ✅ БЫЛО: from datetime import datetime, timezone
...
**✅ ИСПРАВЛЕНО:** Удален локальный импорт `from datetime import datetime, timezone` на строке 5795 (теперь 5798)

---

## 3. position_manager.py (строка ~1266: datetime.now() без timezone)

```python
# src/strategies/scalping/futures/position_manager.py

# Строки 1262-1276: Исправление datetime.now()
time_since_open = 0.0
if position_open_time:
    if isinstance(position_open_time, datetime):
        ...
        time_since_open = (
            datetime.now(timezone.utc) - position_open_time  # ✅ БЫЛО: datetime.now()
        ).total_seconds()
    else:
        try:
...
**✅ ИСПРАВЛЕНО:** Все `datetime.now()` заменены на `datetime.now(timezone.utc)`

---

## 4. orchestrator.py (строки 405-419: ExitAnalyzer)

```python
# src/strategies/scalping/futures/orchestrator.py

# Строки 405-414: Создание ExitAnalyzer
...
**✅ ИСПРАВЛЕНО:** Добавлен вызов `set_exit_analyzer()` после создания ExitAnalyzer

---

## 5. position_manager.py (строки 116-119: set_exit_analyzer)
...
**✅ ДОБАВЛЕНО:** Новый метод для установки ExitAnalyzer
