...
def analyze_log_content(content, log_source):
    ...
    stats = {
        ...
        "sl_closes": [],
        "max_pnl": defaultdict(
            lambda: {"max": float("-inf"), "time": None, "min": float("inf")}
        ),
        "current_pnl": {},
        "errors": [],
        "warnings": 0,
    ...
    for line in lines:
        # Открытие позиций
        if (
            "открыта" in line.lower()
            and "позиция" in line.lower()
            and "entrymanager" in line.lower()
        ):
        ...
        if "позиция.*закрыта" in line.lower() or "closed" in line.lower():
            ...
            if match:
                ...
                reason_match = re.search(
                    r"(profit_drawdown|profit_harvesting|take_profit|stop_loss|sl|tp)",
                    line,
                    re.I,
                )
                if symbol_match:
        ...
        if pnl_match:
            ...
            if symbol_match and time_match:
                ...
                stats["current_pnl"][symbol] = {"pnl": pnl, "time": time_str}
...
def main():
    ...
    current_logs = ["info_2025-11-29.log", "futures_main_2025-11-29.log"]
    ...
    for log_file in current_logs:
        ...
        if log_path.exists():
            ...
            with open(log_path, "r", encoding="utf-8", errors="ignore") as f:
                ...
                all_stats["profit_drawdown_closes"].extend(
                    stats["profit_drawdown_closes"]
                )
                all_stats["profit_harvesting_closes"].extend(
                    stats["profit_harvesting_closes"]
                )
    ...
    zip_files = sorted(
        [
            ...
            if "06-3" in f.name or int(f.name.split("_")[2].split("-")[3]) >= 6
        ]
    )
