...
# -*- coding: utf-8 -*-
"""
СИМУЛЯЦИЯ ТОРГОВЛИ С УЧЕТОМ МОДЕРНИЗАЦИИ
Расчет ожидаемых результатов после модернизации бота
"""
...
BEFORE = {
    ...
    "trades_per_hour": 0.3,  # Сделок в час
    "commission_rate": 0.0005,  # 0.05% (market ордера)
    "tp_percent": 1.0,  # 1.0% TP
    "sl_percent": 1.5,  # 1.5% SL
    "win_rate": 0.60,  # 60% выигрышных сделок (из реальных данных)
    "avg_profit_per_trade": 0.01,  # $0.01 средняя прибыль (из реальных данных)
    "avg_loss_per_trade": -0.09,  # -$0.09 средний убыток (из реальных данных)
}

# Параметры после модернизации
AFTER = {
    "position_size_usd": 125.0,  # Средний размер позиции ($100-150)
    "trades_per_hour": 100.0,  # Сделок в час (80-120/час)
    ...
    "avg_profit_per_trade": 0.50,  # $0.50 средняя прибыль (больше размер позиции)
    "avg_loss_per_trade": -0.30,  # -$0.30 средний убыток (больше размер позиции)
}

# Параметры по режиму рынка
REGIME_PARAMS = {
    "trending": {
        "trades_per_hour": 25,  # Сделок в час на пару
        "position_size_multiplier": 1.1,  # +10% размер позиции
        "tp_percent": 1.1,  # 1.1% TP (больше потенциал)
        "sl_percent": 1.5,  # 1.5% SL
        "win_rate": 0.70,  # 70% выигрышных сделок (больше уверенности)
        "time_distribution": 0.30,  # 30% времени в тренде
    },
    "ranging": {
        "trades_per_hour": 15,  # Сделок в час на пару
    ...
    "choppy": {
        "trades_per_hour": 6,  # Сделок в час на пару
        "position_size_multiplier": 0.8,  # -20% размер позиции
        "tp_percent": 0.9,  # 0.9% TP (быстрый выход)
        "sl_percent": 1.5,  # 1.5% SL
...
def calculate_position_size(
    ...
    base_size = BALANCE_PARAMS[balance_profile]["base_position_usd"]
    ...
    max_size = BALANCE_PARAMS[balance_profile]["max_position_usd"]
...
def calculate_tp_sl(
    ...
    if regime == "trending":
        regime_tp_mult = 1.1  # +10% для trending
    elif regime == "ranging":
        regime_tp_mult = 1.0  # Стандарт для ranging
    else:  # choppy
        regime_tp_mult = 0.9  # -10% для choppy
...
def simulate_trading(
    ...
    profit_per_win = position_size_usd * (tp_percent / 100) - (
        position_size_usd * 0.0002 * 2
    )  # 2 комиссии (вход + выход)
    ...
    # Комиссия на сделку
    commission_per_trade = position_size_usd * 0.0002 * 2  # 2 комиссии (вход + выход)
    ...
    return SimulationResult(
        ...
        win_rate=win_rate,
        avg_profit_per_trade=profit_per_win,
        avg_loss_per_trade=loss_per_loss,
...
def calculate_before_after_comparison():
    ...
    print("=" * 100)
    print("СИМУЛЯЦИЯ ТОРГОВЛИ С УЧЕТОМ МОДЕРНИЗАЦИИ")
    print("=" * 100)
    print(f"\nБаланс: ${BALANCE:.2f}")
    ...
    # Результаты до модернизации
    print("\n" + "=" * 100)
    print("ДО МОДЕРНИЗАЦИИ")
    print("=" * 100)
    ...
    before_avg_profit = BEFORE["avg_profit_per_trade"]
    before_avg_loss = BEFORE["avg_loss_per_trade"]

    # Расчет прибыли/убытка
    before_profit_per_win = before_position_size * (BEFORE["tp_percent"] / 100) - (
        before_position_size * before_commission_rate * 2
    )
    before_loss_per_loss = before_position_size * (BEFORE["sl_percent"] / 100) + (
        before_position_size * before_commission_rate * 2
    )
    before_avg_profit_per_trade = (before_profit_per_win * before_win_rate) + (
        before_loss_per_loss * (1 - before_win_rate)
    )
    before_net_profit_per_hour = before_avg_profit_per_trade * before_trades_per_hour
    ...
    print(f"Частота сделок: {before_trades_per_hour:.1f} сделок/час")
    print(f"Комиссия: {before_commission_rate * 100:.3f}% (market ордера)")
    print(f"Win rate: {before_win_rate * 100:.1f}%")
    print(f"Средняя прибыль на сделку: ${before_profit_per_win:.2f}")
    print(f"Средний убыток на сделку: ${before_loss_per_loss:.2f}")
    print(f"Чистая прибыль на сделку: ${before_avg_profit_per_trade:.4f}")
    ...
    for regime in REGIMES:
        ...
        for result in results_by_regime[regime]:
            print(
                ...
                f"размер ${result.position_size_usd:.2f}, "
    ...
    improvement_profit = (
        ((total_profit_per_hour / before_net_profit_per_hour) - 1) * 100
    ...
    print(
        ...
        f"({before_trades_per_hour:.1f} -> {total_trades_per_hour:.1f} сделок/час)"
    )
    print(
    ...
    if before_net_profit_per_hour > 0:
        ...
        print(f"[OK] Прибыль стала положительной")
    print(
