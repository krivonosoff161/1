...
def analyze_all_logs():
    ...
    analyzed_count = 0
    for zip_file in sorted(zip_files, key=lambda x: x.stat().st_mtime, reverse=True)[:20]:
        try:
                ...
                for log_name in z.namelist():
                    if log_name.endswith('.log'):
                        with z.open(log_name) as f:
                            ...
                            analyze_log_content(content, errors, critical_closes, zero_duration_issues)
                            analyzed_count += 1
    ...
    if errors:
        ...
        for error_type, occurrences in sorted(errors.items(), key=lambda x: len(x[1]), reverse=True)[:10]:
    ...
    try:
        with open(log_path, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            while True:
                ...
                if not chunk:
                    break
                analyze_log_content(chunk, errors, critical_closes, zero_duration_issues)
    except Exception as e:
