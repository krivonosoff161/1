...
class AdaptiveParameterTester:
    ...
    def _create_adaptive_scenarios(self) -> List[AdaptiveContext]:
        ...
        logger.info(f"🎭 Создано {len(scenarios)} адаптивных сценариев (плавная адаптация)")
        return scenarios
    ...
    def simulate_adaptive_params(self, scenario: AdaptiveContext, base_params: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        balance_factor_tp, balance_factor_sl = self._calculate_balance_adaptation_factors(scenario.balance)
    ...
    async def test_adaptive_combination(self, combination: Dict[str, Any]) -> Dict[str, Any]:
        ...
        adaptive_params = self.simulate_adaptive_params(scenario, combination['base_params'])

        # Симулируем торговлю
        results = self._simulate_adaptive_trading(historical_trades, adaptive_params, scenario)
    ...
    def _simulate_adaptive_trading(self, trades: List[Dict[str, Any]], adaptive_params: Dict[str, Any], scenario: AdaptiveContext) -> Dict[str, Any]:
            ...
            if len(selected_combinations) < max_tests:
                ...
                for balance in balances:
                    ...
                    if available > 0:
                        ...
                        selected_combinations.extend(group_combos[per_balance:per_balance + add_count])
                        remaining -= add_count

            combinations = selected_combinations[:max_tests]

        logger.info(f"📊 Тестируем {len(combinations)} комбинаций (из {len(self.adaptive_scenarios) * 4} доступных)")
    ...
    def analyze_adaptive_results(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for range_name, group_results in balance_ranges.items():
            if group_results:
                ...
                tp_multipliers = [r['adaptive_params']['tp_atr_multiplier'] for r in group_results]
...
async def run_adaptive_parameter_tests():
    ...
    results = await tester.run_adaptive_tests(max_tests=20)  # Ограничено для быстрого теста
    ...
    for i, result in enumerate(analysis['top_performers'][:3], 1):
        ...
        print(f"{i}. Баланс:${scenario['balance']}, P&L:{scenario['current_pnl']}%, "
