        ...
        try:
            ...
            if orchestrator:
                try:
                    ...
                    logger.debug(f"⚠️ MarginMonitor: Не удалось получить баланс из orchestrator: {e}")
            ...
            if (current_balance == 0.0 or used_margin == 0.0) and data_registry:
                try:
                    ...
                    logger.debug(f"⚠️ MarginMonitor: Не удалось получить баланс из data_registry: {e}")
            
            # Fallback: если не получили данные
            if current_balance == 0.0:
                logger.warning("⚠️ MarginMonitor: Не удалось получить баланс, используем fallback 1000.0")
            ...
            max_margin_ratio = self.config.get("max_margin_ratio", 0.8)  # Из конфига или 80%
