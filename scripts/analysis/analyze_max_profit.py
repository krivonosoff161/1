            ...
            for symbol in ['SOL-USDT', 'DOGE-USDT', 'BTC-USDT', 'XRP-USDT', 'ETH-USDT']:
                if symbol in line:
                    ...
                    for pattern in patterns:
                        ...
                        if match:
                            try:
                                pnl = float(match.group(1) if len(match.groups()) == 1 else match.group(2))
...
def analyze_zip_archive(zip_path):
    ...
    max_profits = defaultdict(lambda: {'max': -999999, 'time': None, 'min': 999999, 'min_time': None})
...
def main():
    ...
    # Анализируем архивы
    print("\n📦 Анализ архивов...")
    zip_files = sorted(logs_dir.glob('*.zip'), key=lambda x: x.stat().st_mtime, reverse=True)
    
    all_max_profits = defaultdict(lambda: {'max': -999999, 'time': None, 'archives': []})
