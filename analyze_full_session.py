...
LOGS_DIR = Path(r"C:\Users\krivo\simple trading bot okx\logs\futures\archived\logs_2025-12-01_21-39-44")
...
def generate_report(trades):
    ...
    report.append("-" * 90)
    report.append(f"{'Символ':<12} {'Сделок':<8} {'Win':<6} {'Loss':<6} {'WinRate':<10} {'Net PnL':<15} {'Комиссии':<12}")
    report.append("-" * 90)
    ...
    for symbol in sorted(by_symbol.keys(), key=lambda s: sum(t.get("net_pnl", 0) for t in by_symbol[s]), reverse=True):
        ...
        report.append(f"{symbol:<12} {len(tlist):<8} {w:<6} {l:<6} {wr:<10.1f}% ${pnl:+.2f}       ${comm:.2f}")
    ...
    report.append(f"LONG:  {len(longs)} сделок, Win: {long_wins}, WinRate: {long_wins/len(longs)*100 if longs else 0:.1f}%, PnL: ${long_pnl:+.2f}")
    report.append(f"SHORT: {len(shorts)} сделок, Win: {short_wins}, WinRate: {short_wins/len(shorts)*100 if shorts else 0:.1f}%, PnL: ${short_pnl:+.2f}")
    report.append("")
    ...
    for i, t in enumerate(sorted(wins, key=lambda x: -x.get("net_pnl", 0))[:15], 1):
        report.append(f"{i:2d}. {t['symbol']:<10} {t['side']:<6} ${t.get('entry_price',0):.2f} → ${t.get('exit_price',0):.2f} | PnL: ${t['net_pnl']:+.2f} | {t.get('reason','?')}")
