...
class AdaptiveRegimeManager:
    ...
    async def is_signal_valid(self, signal: Dict, market_data=None) -> bool:
        ...
        try:
            ...
            # ✅ ДОПОЛНИТЕЛЬНОЕ УМЯГЧЕНИЕ: Для ranging режима снижаем порог еще на 20%
            # Это позволит больше сигналов проходить в боковике
            if self.current_regime == RegimeType.RANGING:
                min_strength *= 0.8  # Дополнительное снижение на 20% для ranging
                logger.debug(
