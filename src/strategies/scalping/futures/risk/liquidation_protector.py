...
class LiquidationProtector:
    ...
    def check_liquidation_risk(
        ...
        """
        Синхронная проверка риска ликвидации (для обратной совместимости).
        ...
        """
        # ✅ ИСПРАВЛЕНИЕ: Для синхронной версии используем упрощенную проверку
        # В реальной реализации нужно использовать async версию
        logger.debug(
            f"⚠️ LiquidationProtector.check_liquidation_risk (sync) для {symbol}: "
            f"используется упрощенная проверка"
        )
        return True  # По умолчанию разрешаем торговлю (для обратной совместимости)
