...
EXTRACTED_DIR = Path(r"C:\Users\krivo\simple trading bot okx\logs\futures\archived\extracted_2025-12-01_21-39-44")
                ...
                if match:
                    ...
                    for j in range(i+1, min(i+20, len(lines))):
                        ...
                        if "Время закрытия:" in detail_line:
                            m = re.search(r"Время закрытия: (\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})", detail_line)
                            if m:
                        ...
                        if "Длительность удержания:" in detail_line:
                            m = re.search(r"Длительность удержания: ([+-]?\d+\.?\d*) сек", detail_line)
                            if m:
...
def generate_report(trades):
    ...
    report.append("-" * 100)
    report.append(f"{'Символ':<15} {'Сделок':<10} {'Win':<8} {'Loss':<8} {'WinRate':<10} {'Net PnL':<15} {'Комиссии':<12}")
    report.append("-" * 100)
    ...
    for symbol, stats in sorted(symbols_stats.items(), key=lambda x: x[1]["net_pnl"], reverse=True):
        ...
        report.append(f"{symbol:<15} {len(trades_list):<10} {wins_s:<8} {losses_s:<8} {win_rate:<10.1f}% ${stats['net_pnl']:+.4f}      ${stats['commission']:.4f}")
    ...
    report.append(f"LONG:  {len(longs)} сделок, Win: {long_wins}, WinRate: {long_wins/len(longs)*100 if longs else 0:.1f}%, PnL: ${long_pnl:+.4f}")
    report.append(f"SHORT: {len(shorts)} сделок, Win: {short_wins}, WinRate: {short_wins/len(shorts)*100 if shorts else 0:.1f}%, PnL: ${short_pnl:+.4f}")
    report.append("")
    
    # Топ прибыльных сделок
    report.append("🏆 ТОП-20 ПРИБЫЛЬНЫХ СДЕЛОК:")
    report.append("-" * 100)
    sorted_wins = sorted([t for t in trades if t["net_pnl"] and t["net_pnl"] > 0], key=lambda x: -x["net_pnl"])[:20]
    for i, t in enumerate(sorted_wins, 1):
        report.append(f"{i:2d}. {t['symbol']:<12} {t['side']:<6} Entry: ${t['entry_price']:.4f} → Exit: ${t['exit_price']:.4f} | Net PnL: ${t['net_pnl']:+.4f} | {t['reason']}")
    report.append("")
    
    # Топ убыточных сделок
    report.append("💀 ТОП-20 УБЫТОЧНЫХ СДЕЛОК:")
    report.append("-" * 100)
    sorted_losses = sorted([t for t in trades if t["net_pnl"] and t["net_pnl"] < 0], key=lambda x: x["net_pnl"])[:20]
    ...
    report.append("-" * 120)
    report.append(f"{'#':<5} {'Время':<20} {'Символ':<12} {'Side':<6} {'Entry':<12} {'Exit':<12} {'Net PnL':<12} {'Причина':<20}")
    report.append("-" * 120)
    ...
    for i, t in enumerate(trades, 1):
        ...
        reason = t.get("reason", "?")
        report.append(f"{i:<5} {close_time:<20} {t['symbol']:<12} {t['side']:<6} {entry:<12} {exit_p:<12} {pnl:<12} {reason:<20}")
