...
class ExchangeTradesAnalyzer:
    ...
    def _group_long_positions(self, symbol: str, trades: List[Dict]):
        ...
        open_positions = (
            []
        )  # Список: {entry_price, remaining_size, entry_time, entry_order_id, entry_fee, closing_fills}
            ...
            if side == "buy":
                ...
                while remaining_to_close > 0.000001 and open_positions:
                    ...
                    if entry["remaining_size"] > 0:
                        entry_fee_part = entry["entry_fee"] * (
                            close_size / (entry["remaining_size"] + close_size)
                        )
                    else:
                        entry_fee_part = 0
                    ...
                    if abs(close_size - entry["remaining_size"]) < 0.000001:
                        ...
                        total_exchange_pnl = (
                            sum(f["fill_pnl"] for f in entry["closing_fills"])
                            if entry["closing_fills"]
                            else None
                        )
                        total_exit_fee = (
                            sum(f["fee"] for f in entry["closing_fills"])
                            if entry["closing_fills"]
                            else fee
                        )
                        ...
                        position = {
                            ...
                            "net_pnl": (
                                total_exchange_pnl
                                - (entry["entry_fee"] + total_exit_fee)
                            )
                            if total_exchange_pnl is not None
                            else (
                                calculated_pnl - (entry["entry_fee"] + total_exit_fee)
                            ),
                        ...
                        position = {
                            ...
                            "calculated_pnl": calculated_pnl,
                            "net_pnl": (fill_pnl - (entry_fee_part + fee))
                            if fill_pnl is not None
                            else (calculated_pnl - (entry_fee_part + fee)),
                            "is_partial": True,
                        }
    ...
    def _check_position_correctness(self, position: Dict):
        ...
        fee_percent = (
            (position["total_fee"] / position_value * 100) if position_value > 0 else 0
        )
        if (
            fee_percent > 10
        ):  # Увеличил порог до 10% - 5% может быть нормально для очень маленьких позиций
            issues.append(
                {
                    "type": "high_fee",
    ...
    def generate_report(self) -> str:
        ...
        report.append(
            f"   Прибыльных: {profitable} ({profitable/total_positions*100:.1f}%)"
        )
