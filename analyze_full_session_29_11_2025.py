...
if trades_file.exists():
    ...
    print(f"  Прибыльных: {len(df[df['net_pnl'] > 0])} ({len(df[df['net_pnl'] > 0])/len(df)*100:.1f}%)")
    print(f"  Убыточных: {len(df[df['net_pnl'] < 0])} ({len(df[df['net_pnl'] < 0])/len(df)*100:.1f}%)")
    print(f"  Безубыточных: {len(df[df['net_pnl'] == 0])}")
    
    print(f"\nСредняя длительность позиций:")
    print(f"  Средняя: {df['duration_sec'].mean():.1f}с ({df['duration_sec'].mean()/60:.1f} мин)")
    print(f"  Медиана: {df['duration_sec'].median():.1f}с ({df['duration_sec'].median()/60:.1f} мин)")
    print(f"  Минимум: {df['duration_sec'].min():.1f}с ({df['duration_sec'].min()/60:.1f} мин)")
    print(f"  Максимум: {df['duration_sec'].max():.1f}с ({df['duration_sec'].max()/60:.1f} мин)")
    ...
    top_profitable = df.nlargest(5, 'net_pnl')[['symbol', 'side', 'net_pnl', 'reason', 'duration_sec']]
    for idx, row in top_profitable.iterrows():
        print(f"  {row['symbol']} {row['side']}: {row['net_pnl']:.2f} USDT ({row['reason']}, {row['duration_sec']/60:.1f} мин)")
    
    print(f"\nТоп-5 убыточных позиций:")
    top_losses = df.nsmallest(5, 'net_pnl')[['symbol', 'side', 'net_pnl', 'reason', 'duration_sec']]
...
if log_file.exists():
    ...
    ph_failed_pattern = r'❌ PH для (\w+-USDT): (Прибыль недостаточна|Превышен time_limit)'
    ...
    print(f"\nПо символам:")
    for symbol, count in sorted(ph_failed_by_symbol.items(), key=lambda x: x[1], reverse=True):
        print(f"  {symbol}: {count}")
    
    # Поиск случаев с отрицательным PnL
    negative_pnl_pattern = r'❌ PH для (\w+-USDT): Прибыль недостаточна \(\$(-?\d+\.\d+) <'
    ...
    time_limit_pattern = r'❌ PH для (\w+-USDT): Превышен time_limit \((\d+\.\d+)с >= 300с\)'
    ...
    for symbol, times in time_limit_by_symbol.items():
        ...
        print(f"  {symbol}: среднее время = {avg_time:.1f}с ({avg_time/60:.1f} мин) (всего проверок: {len(times)})")
...
if log_file.exists():
    ...
    print(f"\nПо символам:")
    for symbol, count in sorted(no_peak_by_symbol.items(), key=lambda x: x[1], reverse=True):
    ...
    for symbol, pnls in update_peak_by_symbol.items():
        ...
        print(f"    Прибыльных: {len(positive_pnls)} (макс: ${max(positive_pnls) if positive_pnls else 0:.4f})")
        print(f"    Убыточных: {len(negative_pnls)} (мин: ${min(negative_pnls) if negative_pnls else 0:.4f})")
        if positive_pnls:
...
if trades_file.exists():
    ...
    if len(max_holding_closes) > 0:
        ...
        print(f"  Средняя: {max_holding_closes['duration_sec'].mean():.1f}с ({max_holding_closes['duration_sec'].mean()/60:.1f} мин)")
        print(f"  Медиана: {max_holding_closes['duration_sec'].median():.1f}с ({max_holding_closes['duration_sec'].median()/60:.1f} мин)")
        ...
        print(f"  Прибыльных: {len(max_holding_closes[max_holding_closes['net_pnl'] > 0])}")
        print(f"  Убыточных: {len(max_holding_closes[max_holding_closes['net_pnl'] < 0])}")
...
print("   - Обновлять peak_profit даже для убыточных позиций (отслеживать минимальный убыток)")
print("   - Или обновлять peak_profit при любом изменении PnL (не только при увеличении)")
