...
class FuturesScalpingOrchestrator:
    ...
    def __init__(self, config: BotConfig):
        ...
        self.margin_calculator = MarginCalculator(
            ...
            maintenance_margin_ratio=config.futures.get('maintenance_margin_ratio', 0.01),
        ...
        self.order_executor = FuturesOrderExecutor(config, self.client, self.slippage_guard)
        self.position_manager = FuturesPositionManager(config, self.client, self.margin_calculator)
    ...
    async def _initialize_client(self):
        ...
        try:
            ...
            for symbol in self.scalping_config.symbols:
                try:
                    await self.client.set_leverage(symbol, self.config.futures.get('leverage', 3))
                    logger.info(f"✅ Плечо {self.config.futures.get('leverage', 3)}x установлено для {symbol}")
                except Exception as e:
    ...
    async def _validate_signal(self, signal: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if max_size < min_size:
                logger.warning(f"Максимальный размер позиции {max_size:.6f} меньше минимального {min_size:.6f}")
                return False
    ...
    async def _execute_signal(self, signal: Dict[str, Any]):
        ...
        try:
            ...
            if result.get('success'):
                ...
                logger.error(f"❌ Ошибка исполнения сигнала {symbol}: {result.get('error')}")
    ...
    async def _on_liquidation_warning(self, level: str, symbol: str, side: str, 
                                    margin_ratio: float, details: Dict[str, Any]):
        """Обработка предупреждений о ликвидации"""
        try:
            if level == 'critical':
                logger.critical(f"🚨 КРИТИЧЕСКОЕ ПРЕДУПРЕЖДЕНИЕ: {symbol} {side} - маржа: {margin_ratio:.1f}%")
