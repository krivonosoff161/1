# 📋 Код для финальной проверки перед боевым запуском

## 📂 1. candle_patterns.py (полный файл)
...
**Ключевые моменты:**
- ✅ Адаптивная фильтрация через ATR (`atr_factor` параметр)
- ✅ Пороги на основе волатильности (без хардкода)
- ✅ Методы: `is_hammer()`, `is_engulfing_bearish()`, `is_engulfing_bullish()`

---

## 📂 2. liquidity_levels.py (полный файл)
...
**Ключевые моменты:**
- ✅ Структура возврата: `{"below": {...}, "above": {...}}`
- ✅ Кеширование на 30 секунд
- ✅ Анализ стакана (первые 10 уровней)

---

## 📂 3. exit_analyzer.py (только новые/измененные части)

### 3.1. Импорты (строки 8-20)
...
### 3.2. Инициализация модулей (строки 72-100)
...
### 3.3. Вставка в _generate_exit_for_trending (строки 1312-1330)
...
if pnl_percent < 0:
    ...
    # ... существующий код hold ...
```

### 3.4. Вставка в _generate_exit_for_ranging (строки 1631-1649)
...
### 3.5. Вставка в _generate_exit_for_choppy (строки 1881-1899)
...
### 3.6. Методы получения данных (строки 1961-2060)
...
### 3.7. Методы проверки индикаторов (строки 2062-2242)
...
async def _check_liquidity_sweep(self, symbol: str, side: str) -> int:
    """Проверка ликвидности (если ниже/выше нас еще ликвидность → шанс на отскок)"""
...
### 3.8. Основной метод принятия решения (строки 2260-2311)
...
async def _should_force_close_by_smart_analysis(
    ...
    for i, result in enumerate(results):
        if isinstance(result, Exception):
            logger.debug(f"⚠️ Ошибка проверки индикатора {i}: {result}")
            scores.append(0)
        else:
...
### Адаптивность:
- ✅ ATR используется для фильтрации в `candle_patterns.py` (строки 85-91, 172-178)
- ✅ ATR используется для tolerance в `_check_pivot_support` (строка 2234: `tolerance = atr * 0.3`)
- ✅ Пороги на основе волатильности, без хардкода

### Логика закрытия:
- ✅ Условие: `pnl_percent <= -sl_percent * 1.5` (убыток >= 1.5 * SL)
- ✅ Условие: `reversal_score <= 2` (нет признаков отката)
- ✅ Условие: `trend_against >= 0.7` (тренд против нас усиливается)

### Безопасность:
- ✅ Минимальная вставка (только перед `max_holding_exceeded_but_loss`)
- ✅ Все исключения обрабатываются через `return_exceptions=True`
- ✅ Fallback на `None` если данные недоступны

### Параллельность:
- ✅ `asyncio.gather(*tasks, return_exceptions=True)` - не блокирует цикл

### Логирование:
- ✅ Подробные логи в `_should_force_close_by_smart_analysis`
- ✅ Debug логи в каждом методе проверки

### TODO/Fallback:
- ✅ `_get_correlation()` возвращает `None` (не сломает выполнение)
- ✅ Все методы проверки обрабатывают `None` корректно
