...
class TradeComparator:
    ...
    def extract_trades_from_logs(
        self, log_files: List[Path], start_date: datetime, end_date: datetime
    ):
        ...
        for log in parsed_logs:
            ...
            if any(
                x in message.upper()
                for x in ["ПОЗИЦИЯ ОТКРЫТА", "POSITION OPENED", "ПОЗИЦИЯ ОТКРЫТ"]
            ):
            ...
            # Паттерны: "💰 ПОЗИЦИЯ ЗАКРЫТА", "TRADE CLOSED", "Позиция закрыта"
                ...
                x in message.upper()
                for x in ["ПОЗИЦИЯ ЗАКРЫТА", "TRADE CLOSED", "EXIT_HIT"]
            ):
    ...
    def _parse_position_closed(
        ...
        gross_match = re.search(
            r"gross[_\s]?pnl[=:]?\s*\$?([\-\+]?[\d.]+)", message, re.I
        )
        ...
        if not reason_match:
            reason_match = re.search(
                r"закрыт[а]?\s+(?:по|через)\s+(\w+)", message, re.I
            )
    ...
    def _check_match(self, bot_trade: Dict, exchange_trade: Dict) -> Dict:
        ...
        if bot_exit and ex_price:
            ...
            if price_diff > 0.1:  # Более 0.1% разница
                issues.append(
                    f"Цена закрытия: бот={bot_exit:.2f}, биржа={ex_price:.2f} (разница {price_diff:.2f}%)"
                )
                matched = False
        ...
        if bot_size and ex_size:
            ...
            if size_diff > 1.0:  # Более 1% разница
                issues.append(
                    f"Размер: бот={bot_size:.6f}, биржа={ex_size:.6f} (разница {size_diff:.2f}%)"
                )
                matched = False
        ...
        if bot_pnl is not None and ex_pnl is not None:
            ...
            if pnl_diff > 0.1:  # Более $0.1 разница
                issues.append(
                    f"PnL: бот={bot_pnl:.2f}, биржа={ex_pnl:.2f} (разница ${pnl_diff:.2f})"
                )
                matched = False
        ...
        if bot_fee and ex_fee:
            ...
            if fee_diff > 0.01:  # Более $0.01 разница
                issues.append(
                    f"Комиссия: бот={bot_fee:.4f}, биржа={ex_fee:.4f} (разница ${fee_diff:.4f})"
                )
    ...
    def generate_report(self) -> str:
        ...
        if self.matched_trades:
            ...
            for i, match in enumerate(self.matched_trades[:10], 1):  # Первые 10
                ...
                report.append(
                    f"   Время: {bot.get('timestamp')} -> {bot.get('exit_time')}"
                )
                report.append(
                    f"   Цена закрытия: бот=${bot.get('exit_price'):.2f}, биржа=${ex.get('price'):.2f}"
                )
                report.append(
                    f"   PnL: бот=${bot.get('net_pnl'):.2f}, биржа=${ex.get('pnl'):.2f}"
                )
                report.append("")

            if len(self.matched_trades) > 10:
                report.append(
                    f"   ... и еще {len(self.matched_trades) - 10} совпадающих сделок"
                )
                report.append("")
        ...
        if self.discrepancies:
            ...
            if len(self.discrepancies) > 10:
                report.append(
                    f"   ... и еще {len(self.discrepancies) - 10} расхождений"
                )
                report.append("")
        ...
        if self.bot_only:
            ...
            for i, trade in enumerate(self.bot_only[:10], 1):
                ...
                report.append(
                    f"   Время: {trade.get('timestamp')} -> {trade.get('exit_time')}"
                )
                report.append(
        ...
        if self.exchange_only:
            ...
            for i, trade in enumerate(self.exchange_only[:10], 1):
                ...
                report.append(
                    f"   Цена: ${trade.get('price'):.2f}, Размер: {trade.get('size'):.6f}"
                )
                report.append("")
...
def main():
    ...
    details_file.write_text(
        json.dumps(details, indent=2, ensure_ascii=False, default=str), encoding="utf-8"
    )
