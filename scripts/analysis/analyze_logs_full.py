...
def analyze_logs(log_file: Path):
    ...
    with open(log_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            # Открытие позиций
            if (
                "POSITION OPENED" in msg
                or "Позиция открыта" in msg
                or "Открыта позиция" in msg
            ):
                stats["positions_opened"].append(
                    {"time": parsed["timestamp"], "message": msg}
                )

            # Закрытие позиций
            if (
                "TRADE CLOSED" in msg
                or "Позиция закрыта" in msg
                or "закрыта по причине" in msg
            ):
            ...
            # Сигналы
            if (
                "Сгенерирован сигнал" in msg
                or "SIGNAL" in msg
                or "сигнал для" in msg
                or "Основной цикл: сгенерировано" in msg
            ):
            ...
            # Блокировка сигналов
            if (
                "сигнал ОТМЕНЕН" in msg
                or "сигнал заблокирован" in msg
                or "БЛОКИРУЕМ" in msg
            ):
            ...
            if parsed["level"] in ["ERROR", "CRITICAL"]:
                stats["errors"].append({"time": parsed["timestamp"], "message": msg})

            # Предупреждения
            if parsed["level"] == "WARNING":
                stats["warnings"].append({"time": parsed["timestamp"], "message": msg})
...
def print_report(stats: dict):
    ...
    if stats["signals_generated"]:
        ...
        cycles = [
            item
            for item in stats["signals_generated"]
            if "Основной цикл" in item["message"]
        ]
        if cycles:
    ...
    if stats["adx_trends"]:
        ...
        for symbol, items in sorted(adx_by_symbol.items()):
            ...
            if items:
                ...
                if adx_match:
                    print(
                        f"         Последний ADX: {adx_match.group(1)} ({last['time']})"
                    )
    print()
    ...
    safe_count = sum(
        1 for item in stats["margin_checks"] if "safe=True" in item["message"]
    )
    unsafe_count = sum(
        1 for item in stats["margin_checks"] if "safe=False" in item["message"]
    )
