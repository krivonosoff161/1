...
signal_pattern = re.compile(
    r"✅.*?(LONG|SHORT|BEARISH|BULLISH).*?сигнал.*?для (\w+-\w+)"
)
...
position_block_pattern = re.compile(
    r"(MaxSizeLimiter|CorrelationFilter|уже.*?позиция|лимит.*?позиций)"
)
...
for i, line in enumerate(lines):
    ...
    if (
        "✅" in line
        and ("сигнал" in line or "MA" in line)
        and (
            "LONG" in line or "SHORT" in line or "BEARISH" in line or "BULLISH" in line
        )
    ):
    ...
    if current_signal and "✅" in line and current_signal["symbol"] in line:
        for filter_name in [
            ...
            if (
                ...
                and "проходит" in line.lower()
                or "подтверждён" in line.lower()
                or "разрешён" in line.lower()
                or "бонус" in line.lower()
            ):
            ...
            if filter_name.lower() in line.lower():
                if filter_name not in current_signal["filters_blocked"]:
                    ...
                    if "объём" in line.lower() or "volume" in line.lower():
                        current_signal[
                            "block_reason"
                        ] = "Недостаточный объём ликвидности"
                        ...
                        current_signal[
                            "block_reason"
                        ] = "Нет подтверждения на старшем таймфрейме"
                    elif "maxsize" in line.lower() or "лимит" in line.lower():
                        current_signal[
                            "block_reason"
                        ] = "Превышен лимит размера/количества позиций"
    ...
    if current_signal:
        ...
        if i < len(lines) - 1:
            ...
            if (
                "✅" in next_line and ("сигнал" in next_line or "MA" in next_line)
            ) or i > current_signal["line_num"] + 50:
                if (
                    current_signal["filters_passed"]
                    or current_signal["filters_blocked"]
                ):
...
if current_signal and (
    current_signal["filters_passed"] or current_signal["filters_blocked"]
):
    signals.append(current_signal)
...
passed_all = [
    s for s in signals if not s["filters_blocked"] and len(s["filters_passed"]) >= 3
]
...
if passed_all:
    ...
    for s in passed_all[:5]:
        print(
            f"   {s['symbol']} {s['direction']}: прошёл {len(s['filters_passed'])} фильтров"
        )
        if s.get("final_score"):
            print(f"      Score: {s['final_score']}")
        if s.get("position_blocked"):
            print(
                f"      ⚠️ Блокировка на уровне позиции: {s['position_blocked'][:100]}"
            )
...
for i, line in enumerate(lines):
    if any(
        x in line.lower()
        for x in ["открыт", "позиция", "ордер", "размещён", "execute", "place"]
    ):
