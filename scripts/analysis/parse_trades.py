...
class TradesParser:
    ...
    async def get_fills(
        ...
        params = {"instType": "SWAP", "limit": str(limit)}  # Фьючерсы
    ...
    async def get_all_fills_period(
        ...
        while True:
            ...
            fills = await self.get_fills(
                symbol=symbol, start_time=start_time, end_time=current_end, limit=limit
            )
    ...
    def format_trade(self, fill: Dict) -> Dict:
        ...
        return {
            ...
            if fill.get("fillPnl") and fill.get("fillPnl") != "0"
            else None,
        }
