...
def analyze_close_reasons(log_file_path: str) -> Dict:
    ...
    close_reasons = collections.Counter(row.get('reason', 'unknown') for row in positions)
    ...
    if pnl_values:
        print(f'💰 P&L: min={min(pnl_values):.2f}, max={max(pnl_values):.2f}, avg={sum(pnl_values)/len(pnl_values):.2f}')
    ...
    for reason, count in close_reasons.items():
        ...
        reason_pnl = [float(p.get('net_pnl', 0)) for p in reason_positions if p.get('net_pnl')]
    ...
    for symbol in symbols:
        ...
        symbol_reasons = collections.Counter(p.get('reason', 'unknown') for p in symbol_positions)
    ...
    return {
        ...
        'symbols_analysis': {symbol: dict(collections.Counter(p.get('reason', 'unknown')
