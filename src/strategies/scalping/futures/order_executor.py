...
class FuturesOrderExecutor:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(f"✅ Подключение к Futures клиенту установлено. Баланс: {balance:.2f} USDT")
    ...
    async def execute_signal(self, signal: Dict[str, Any], position_size: float) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"🎯 Исполнение сигнала: {symbol} {side} размер={position_size:.6f}")
    ...
    async def _execute_order(self, signal: Dict[str, Any], position_size: float) -> Dict[str, Any]:
        """Исполнение ордера"""
        try:
            symbol = signal.get('symbol')
            side = signal.get('side')
            ...
            if order_type == 'market':
                ...
                result = await self._place_limit_order(symbol, side, position_size, price)
            elif order_type == 'oco':
    ...
    def _determine_order_type(self, signal: Dict[str, Any]) -> str:
        ...
        if signal_type in ['rsi_oversold', 'rsi_overbought', 'bb_oversold', 'bb_overbought']:
            return 'market'  # Быстрое исполнение для отскоков
        elif signal_type in ['macd_bullish', 'macd_bearish', 'ma_bullish', 'ma_bearish']:
    ...
    async def _place_market_order(self, symbol: str, side: str, size: float) -> Dict[str, Any]:
    ...
    async def _place_limit_order(self, symbol: str, side: str, size: float, price: float) -> Dict[str, Any]:
        """Размещение лимитного ордера"""
        try:
            logger.info(f"📊 Размещение лимитного ордера: {symbol} {side} {size:.6f} @ {price:.2f}")
    ...
    async def _place_oco_order(self, signal: Dict[str, Any], size: float) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"🎯 Размещение OCO ордера: {symbol} {side} {size:.6f} TP:{tp_price:.2f} SL:{sl_price:.2f}")
    ...
    async def _calculate_tp_sl_prices(self, signal: Dict[str, Any], size: float) -> Tuple[float, float]:
