...
class AuditLogAnalyzer:
    ...
    def __init__(self, logs_dir: Path):
        ...
        self.warning_thresholds = {
            ...
            'exit_slippage': (r'exit.*slippage[:\s]+([\d.]+)', 0.3, 'exit_slippage > 0.3%'),
            'trail_distance': (r'trail.*distance[:\s]+([\d.]+)', 0.05, 'trail_distance < 0.05%'),
    ...
    def check_cast_errors(self, line: str, file: Path, line_num: int):
        ...
        patterns = [
            ...
            (r'position\.get\(["\']size["\']\)\s*[^float]', 'position.get("size") без float()')
        ]
        ...
        for num, name, key in sections:
            ...
            if issues and key != 'missing_logs':
                ...
                for issue in list(issues)[:5]:
                    ...
                    report.append(f"  - {file_name}:{issue.get('line', '?')} - {issue.get('issue', issue.get('threshold', ''))}")
                if len(issues) > 5:
