# 📊 ДАННЫЕ ДЛЯ АНАЛИЗА КИМЫ

## 📋 СОДЕРЖАНИЕ

1. [Логи запуска и работы бота](#1-логи-запуска-и-работы-бота)
2. [Конфигурационные файлы](#2-конфигурационные-файлы)
3. [Файлы позиций](#3-файлы-позиций)
4. [Код ExitAnalyzer](#4-код-exitanalyzer)
5. [Код PositionSizer (RiskManager)](#5-код-positionsizer-riskmanager)
6. [Код TrailingStopLoss](#6-код-trailingstoploss)

---

## 1. ЛОГИ ЗАПУСКА И РАБОТЫ БОТА

### 1.1. Инициализация модулей

```
2025-12-07 22:02:08.480 | INFO | FundingRateMonitor инициализирован в orchestrator
2025-12-07 22:02:08.480 | INFO | ExitAnalyzer инициализирован
2025-12-07 22:02:08.480 | INFO | PositionMonitor инициализирован (check_interval=5.0 сек)
2025-12-07 22:02:36.484 | INFO | TrailingStopLoss инициализирован: entry=0.14215, side=short, trail=2.00%
2025-12-07 22:02:36.488 | INFO | TrailingStopLoss для DOGE-USDT инициализирован: trail=2.000%, fee=0.100%, loss_cut=4.00% от маржи, min_holding=35.0 мин, regime=ranging
```

**Примечание**: Логи инициализации `CandlePatternDetector`, `LiquidityLevelsDetector`, `VolumeProfileCalculator`, `PivotCalculator` отсутствуют, так как они создаются без логирования в `ExitAnalyzer.__init__`.

### 1.2. Логи ExitAnalyzer при анализе позиций

```
2025-12-07 22:02:39.306 | INFO | ExitAnalyzer DOGE-USDT: режим=ranging, источник=signal_generator.regime_managers, metadata.regime=ranging
2025-12-07 22:02:39.306 | INFO | ExitAnalyzer RANGING DOGE-USDT: entry_price=0.14, current_price=0.14, side=short, Gross PnL%=-0.0703%, Net PnL%=-0.0703% (с комиссией)
2025-12-07 22:02:39.306 | INFO | ExitAnalyzer RANGING DOGE-USDT: TP=4.00%, PnL%=-0.0703%, достигнут=False
2025-12-07 22:02:39.306 | INFO | ExitAnalyzer RANGING DOGE-USDT: big_profit_exit=3.00%, PnL%=-0.0703%, достигнут=False
2025-12-07 22:02:39.306 | INFO | ExitAnalyzer RANGING DOGE-USDT: partial_tp enabled=True, trigger_percent=1.00%
2025-12-07 22:02:39.306 | INFO | ExitAnalyzer RANGING DOGE-USDT: Нет причины для закрытия - TP=4.00% (не достигнут), big_profit=3.00% (не достигнут), partial_tp=1.00% (не достигнут), текущий PnL%=-0.07%, время: 0.1 мин / 20.0 мин
```

### 1.3. Логи TrailingStopLoss

```
2025-12-07 22:02:36.484 | INFO | TrailingStopLoss инициализирован: entry=0.14215, side=short, trail=2.00%, entry_time=2025-12-07T22:02:33.277000
2025-12-07 22:02:36.488 | INFO | TrailingStopLoss для DOGE-USDT инициализирован: trail=2.000%, fee=0.100%, loss_cut=4.00% от маржи, min_holding=35.0 мин, regime=ranging
```

### 1.4. Логи PositionMonitor

```
2025-12-07 22:02:29.279 | INFO | PositionMonitor: Запущен
2025-12-07 22:02:29.279 | INFO | PositionMonitor запущен (фоновая задача)
```

### 1.5. Логи DRIFT_ADD / DRIFT_REMOVE

```
2025-12-07 22:02:57.677 | CRITICAL | DRIFT_ADD ETH-USDT found on exchange but not in registry
2025-12-07 22:02:57.677 | WARNING | DRIFT_ADD_SYNCED ETH-USDT force-registered in PositionRegistry
2025-12-07 22:02:57.694 | WARNING | DRIFT_ADD_TSL_CREATED ETH-USDT TSL initialized (entry=3125.7000, side=short, regime=ranging)
```

### 1.6. Логи smart_close / sl_close / tp_close

**Статус**: Не срабатывают, так как убытки слишком маленькие (< -3.0% для smart_close).

### 1.7. Ошибки

```
2025-12-07 22:02:51.218 | ERROR | КРИТИЧЕСКАЯ ОШИБКА: Размер позиции $85.17 превышает max_position_size $70.00 для ETH-USDT! Ограничиваем до $70.00
2025-12-07 22:02:51.221 | ERROR | КРИТИЧЕСКАЯ ОШИБКА: Размер позиции после волатильности $91.00 превышает max_position_size $70.00 для ETH-USDT! Ограничиваем до $70.00
```

**Примечание**: Это нормальные ошибки - система автоматически ограничивает размер позиции до max_position_size.

---

## 2. КОНФИГУРАЦИОННЫЕ ФАЙЛЫ

### 2.1. Micro профиль (config_futures.yaml)

```yaml
micro:  # $100 - $500 - НОВЫЙ ПРОФИЛЬ ДЛЯ МАЛЫХ БАЛАНСОВ
  threshold: 500.0
  min_balance: 100.0
  size_at_min: 30.0
  size_at_max: 50.0
  base_position_usd: 50.0  # Для обратной совместимости
  min_position_usd: 30.0
  max_position_usd: 70.0
  max_open_positions: 5
  max_position_percent: 25.0
  progressive: true  # ✅ ВАРИАНТ B: Прогрессивная адаптация
```

### 2.2. Position Sizing (config_futures.yaml)

```yaml
scalping_config:
  leverage: 5
  sl_percent: 2.0
  tp_percent: 4.0
  risk_per_trade: 1.2  # % от баланса
  volatility_adjustment:
    enabled: true
    base_atr_percent: 2.0
    multiplier_range: [0.5, 1.5]
  adaptive_risk:
    enabled: true
    strength_multipliers:
      weak: 0.5
      medium: 1.0
      strong: 1.3
```

### 2.3. Exit Conditions (config_futures.yaml)

```yaml
adaptive_regime:
  ranging:
    sl_percent: 2.0
    tp_percent: 4.0
    max_holding_minutes: 20.0
    big_profit_exit_percent: 3.0
    partial_tp:
      enabled: true
      trigger_percent: 1.0
      close_percent: 50.0
```

---

## 3. ФАЙЛЫ ПОЗИЦИЙ

### 3.1. positions_open_2025-12-07.csv

```csv
timestamp,symbol,side,entry_price,size,regime,order_id,order_type
2025-12-07T19:02:36.107851,DOGE-USDT,short,0.14215000,490.00000000,ranging,3108205866300018688,limit
2025-12-07T19:02:58.407716,ETH-USDT,short,3125.70000000,0.02200000,ranging,3108206590404661248,limit
2025-12-07T19:03:20.843250,BTC-USDT,short,91268.72656000,0.00070000,ranging,3108207340983750656,limit
```

### 3.2. Registry (PositionRegistry)

**Примечание**: `PositionRegistry` хранит данные в памяти, не сохраняет в JSON файл. Данные доступны через методы:
- `get_all_positions()` - все позиции
- `get_all_metadata()` - все метаданные

---

## 4. КОД EXITANALYZER

### 4.1. __init__ (где создаются индикаторы)

```python
def __init__(
    self,
    position_registry: PositionRegistry,
    data_registry: DataRegistry,
    exit_decision_logger=None,
    orchestrator=None,
    config_manager=None,
    signal_generator=None,
    signal_locks_ref: Optional[Dict[str, asyncio.Lock]] = None,
):
    # ... инициализация ...
    
    # ✅ НОВОЕ: Инициализация модулей для умного закрытия
    self.candle_pattern_detector = CandlePatternDetector()
    self.volume_profile_calculator = VolumeProfileCalculator()
    self.pivot_calculator = PivotCalculator()
    self.liquidity_levels_detector = LiquidityLevelsDetector(client=self.client)
    
    logger.info("✅ ExitAnalyzer инициализирован")
```

### 4.2. analyze_position(...)

```python
async def analyze_position(self, symbol: str) -> Optional[Dict[str, Any]]:
    """Проанализировать позицию и принять решение."""
    analysis_start = time.perf_counter()
    
    # ✅ FIX: Получаем или создаём lock для символа
    if symbol not in self._signal_locks_ref:
        self._signal_locks_ref[symbol] = asyncio.Lock()
    
    async with self._signal_locks_ref[symbol]:
        return await self._analyze_position_impl(symbol, analysis_start)
```

### 4.3. _should_force_close_by_smart_analysis (smart_close)

```python
async def _should_force_close_by_smart_analysis(
    self,
    symbol: str,
    position_side: str,
    pnl_pct: float,
    sl_pct: float,
) -> bool:
    """
    Возвращает True, если:
      - убыток уже значительный (>= 1.5 * SL)
      - ни один индикатор не показывает разворот в нашу пользу
      - тренд усиливается против нас
    """
    tasks = [
        self._check_reversal_signals_score(symbol, position_side),
        self._check_funding_bias(symbol, position_side),
        self._check_correlation_bias(symbol, position_side),
        self._check_liquidity_sweep(symbol, position_side),
        self._check_reversal_candles(symbol, position_side),
        self._check_volume_profile_support(symbol, position_side),
        self._check_pivot_support(symbol, position_side),
    ]
    
    results = await asyncio.gather(*tasks, return_exceptions=True)
    
    reversal_score = 0
    for res in results:
        if isinstance(res, Exception):
            logger.warning(f"⚠️ Ошибка в smart_analysis задаче: {res}")
        else:
            reversal_score += res
    
    # Сила тренда против нас
    trend_data = await self._analyze_trend_strength(symbol)
    trend_against = 0.0
    if trend_data:
        ts = trend_data.get("trend_strength", 0.0)
        direction = trend_data.get("trend_direction", "neutral")
        if (position_side == "long" and direction == "bearish") or (
            position_side == "short" and direction == "bullish"
        ):
            trend_against = ts
    
    # Принудительное закрытие:
    # 1. нет признаков разворота (score ≤ 2)
    # 2. тренд против нас усиливается (≥ 0.7)
    decision = reversal_score <= 2 and trend_against >= 0.7
    return decision
```

### 4.4. _generate_exit_for_ranging (sl_close / tp_close)

```python
async def _generate_exit_for_ranging(
    self,
    symbol: str,
    position: Dict[str, Any],
    metadata: PositionMetadata,
    current_price: float,
    pnl_percent: float,
    minutes_in_position: float,
) -> Optional[Dict[str, Any]]:
    """Генерация решения для ranging режима."""
    
    # ---------- УМНОЕ ЗАКРЫТИЕ УБЫТОЧНОЙ ПОЗИЦИИ ----------
    if pnl_percent <= -sl_percent * 1.5:
        smart_close = await self._should_force_close_by_smart_analysis(
            symbol, position_side, pnl_percent, sl_percent
        )
        if smart_close:
            return {
                "action": "close",
                "reason": "smart_forced_close_ranging",
                "pnl_pct": pnl_percent,
            }
    # ---------- КОНЕЦ УМНОГО ЗАКРЫТИЯ ----------
    
    # Проверка TP
    if pnl_percent >= tp_percent:
        return {
            "action": "close",
            "reason": "tp_reached_ranging",
            "pnl_pct": pnl_percent,
        }
    
    # Проверка SL
    if pnl_percent <= -sl_percent:
        return {
            "action": "close",
            "reason": "sl_reached_ranging",
            "pnl_pct": pnl_percent,
        }
    
    # ... остальная логика ...
```

---

## 5. КОД POSITIONSIZER (RISKMANAGER)

### 5.1. calculate_position_size(...)

```python
async def calculate_position_size(
    self,
    balance: Optional[float] = None,
    price: float = 0.0,
    signal: Optional[Dict[str, Any]] = None,
    signal_generator=None,
) -> float:
    """Рассчитывает размер позиции с учетом Balance Profiles и режима рынка."""
    
    # 1. Получаем баланс
    if balance is None:
        balance = await self._get_balance_from_registry_or_api()
    
    # 2. Проверка max_daily_loss
    if not await self._check_max_daily_loss(balance):
        return 0.0
    
    # 3. Получаем балансовый профиль
    balance_profile = self.config_manager.get_balance_profile(balance)
    
    # 4. ✅ ПРОГРЕССИВНЫЙ РАСЧЕТ
    is_progressive = balance_profile.get("progressive", False)
    if is_progressive:
        size_at_min = balance_profile.get("size_at_min", 30.0)
        size_at_max = balance_profile.get("size_at_max", 50.0)
        min_balance = balance_profile.get("min_balance", 100.0)
        max_balance = balance_profile.get("threshold", 500.0)
        
        # Линейная интерполяция
        if max_balance > min_balance:
            balance_range = max_balance - min_balance
            size_range = size_at_max - size_at_min
            clamped_balance = max(min_balance, min(balance, max_balance))
            interpolated_size = size_at_min + (
                size_range * (clamped_balance - min_balance) / balance_range
            )
            base_usd_size = interpolated_size
    
    # 5. Применяем множители
    # - Per-symbol multiplier
    # - Regime multiplier
    # - Signal strength multiplier
    # - Volatility multiplier
    
    # 6. Ограничиваем до max_position_usd
    margin_required = base_usd_size / leverage
    margin_usd = max(min_margin_usd, min(margin_required, max_margin_usd))
    
    # 7. Конвертируем в монеты
    position_size = (margin_usd * leverage) / price
    
    return position_size
```

### 5.2. Использование volatility_multiplier, strength_multiplier, progressive sizing

```python
# Volatility multiplier
if volatility_adjustment_enabled:
    current_atr_percent = (atr / price) * 100
    base_atr_percent = volatility_config.get("base_atr_percent", 2.0)
    volatility_multiplier = base_atr_percent / max(current_atr_percent, 0.01)
    volatility_multiplier = max(0.5, min(1.5, volatility_multiplier))
    base_usd_size *= volatility_multiplier

# Strength multiplier
signal_strength = signal.get("strength", 1.0)
strength_multipliers = adaptive_risk_config.get("strength_multipliers", {})
if signal_strength >= 0.8:
    strength_mult = strength_multipliers.get("strong", 1.3)
elif signal_strength >= 0.5:
    strength_mult = strength_multipliers.get("medium", 1.0)
else:
    strength_mult = strength_multipliers.get("weak", 0.5)
base_usd_size *= strength_mult
```

---

## 6. КОД TRAILINGSTOPLOSS

### 6.1. update(...)

```python
def update(
    self,
    current_price: float,
    current_time: Optional[float] = None,
    order_flow_delta: Optional[float] = None,
    trend_strength: Optional[float] = None,
    trend_direction: Optional[str] = None,
) -> Tuple[Optional[float], Optional[str], Optional[Dict[str, Any]]]:
    """
    Обновляет трейлинг стоп и возвращает решение о закрытии.
    
    Returns:
        (stop_price, reason, metadata) или (None, None, None) если не закрывать
    """
    if current_time is None:
        current_time = time.time()
    
    # 1. Обновляем highest_price / lowest_price
    if self.side == "long":
        if current_price > self.highest_price:
            self.highest_price = current_price
    else:  # short
        if current_price < self.lowest_price:
            self.lowest_price = current_price
    
    # 2. Рассчитываем текущий трейлинг
    pnl_percent = self._calculate_pnl_percent(current_price)
    
    # 3. Адаптивный трейлинг на основе прибыли
    if pnl_percent < 0.005:  # < 0.5%
        trail_mult = self.trail_growth_low_multiplier
    elif pnl_percent < 0.015:  # 0.5-1.5%
        trail_mult = self.trail_growth_medium_multiplier
    else:  # > 1.5%
        trail_mult = self.trail_growth_high_multiplier
    
    current_trail = self.initial_trail * trail_mult
    
    # 4. Проверка loss_cut
    if self.loss_cut_percent and pnl_percent <= -self.loss_cut_percent:
        stop_price = self._calculate_stop_price(current_price)
        return stop_price, "loss_cut", {"pnl_percent": pnl_percent}
    
    # 5. Проверка timeout
    if self.timeout_minutes:
        minutes_held = (current_time - self.entry_timestamp) / 60.0
        if minutes_held >= self.timeout_minutes:
            if pnl_percent < 0:
                # В убытке - не закрываем по таймауту
                return None, None, None
            stop_price = self._calculate_stop_price(current_price)
            return stop_price, "timeout", {"minutes_held": minutes_held}
    
    # 6. Проверка min_holding
    if self.min_holding_minutes:
        minutes_held = (current_time - self.entry_timestamp) / 60.0
        if minutes_held < self.min_holding_minutes:
            return None, None, None
    
    # 7. Рассчитываем stop_price
    stop_price = self._calculate_stop_price(current_price)
    
    return stop_price, None, None
```

### 6.2. Логика trailing_distance, activation_threshold

```python
def _calculate_stop_price(self, current_price: float) -> float:
    """Рассчитывает цену стоп-лосса."""
    if self.side == "long":
        # Для лонга: stop = highest_price * (1 - current_trail)
        stop_price = self.highest_price * (1 - self.current_trail)
    else:  # short
        # Для шорта: stop = lowest_price * (1 + current_trail)
        stop_price = self.lowest_price * (1 + self.current_trail)
    
    return stop_price
```

---

## 📌 ИТОГОВЫЕ ЗАМЕЧАНИЯ

### ✅ Работает:
1. Инициализация всех модулей
2. Открытие позиций
3. Анализ позиций (ExitAnalyzer)
4. Ограничение размера позиций
5. Trailing Stop Loss
6. Синхронизация позиций (DRIFT_ADD обработан)

### ⚠️ Предупреждения (нормальные):
1. Превышение max_position_size (автоматически ограничивается)
2. DRIFT_ADD (позиция синхронизирована)

### ❌ Не срабатывает (ожидаемо):
1. Умное закрытие (убытки слишком маленькие < -3.0%)
2. SL/TP закрытие (убытки/прибыль не достигли порогов)

---

**Дата создания**: 2025-12-07
**Версия**: 1.0



