...
class SideUsageChecker:
    ...
    def check_all(self):
        ...
        files = list(self.codebase_path.rglob("src/strategies/scalping/futures/**/*.py"))
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                if re.search(r'\bside\s*==\s*["\'](long|short|LONG|SHORT)', line):
                    if ".lower()" not in line and "side.lower()" not in lines[max(0, i-3):i]:
                ...
                if re.search(r'\bposition_side\s*==\s*["\'](long|short|LONG|SHORT)', line):
                    if ".lower()" not in line:
                        # Проверяем, нормализован ли position_side выше
                        is_safe = self._check_if_position_side_normalized_above(lines, i)
                        if not is_safe:
                            self.issues.append({
                                "file": str(file_path.relative_to(self.codebase_path)),
                                "line": i,
    ...
    def _check_if_side_normalized_above(self, lines: List[str], current_line: int) -> bool:
    ...
    def _check_if_position_side_normalized_above(self, lines: List[str], current_line: int) -> bool:
    ...
    def print_summary(self):
        ...
        if not self.issues:
            ...
            print(f"{YELLOW}⚠️  Найдено {len(self.safe_places)} безопасных использований{RESET}")
        else:
