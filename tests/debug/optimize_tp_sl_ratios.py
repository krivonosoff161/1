...
def optimize_tp_sl_ratios(log_file_path: str) -> Dict:
    ...
    win_rate = (
        len([p for p in positions if float(p.get("net_pnl", 0)) > 0])
        / len(positions)
        * 100
    )
    ...
    for i, result in enumerate(results[:5], 1):
        print(
            f'   {i}. TP:{result["tp_ratio"]} SL:{result["sl_multiplier"]} | P&L:{result["pnl"]:.2f} | WR:{result["win_rate"]:.1f}% | PF:{result["profit_factor"]:.2f}'
        )
    ...
    print(
        f"   Улучшение: {improvement:.2f} USDT ({improvement/current_pnl*100 if current_pnl != 0 else 0:.1f}%)"
    )

    return {
        "best_result": best_result,
        "all_results": results,
        "current_stats": {"pnl": current_pnl, "win_rate": win_rate},
