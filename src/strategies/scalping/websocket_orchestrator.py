...
from src.websocket_manager import WebSocketPriceManager, WebSocketConfig, PriceData, initialize_websocket, get_websocket_manager, get_latency_monitor
from src.balance import AdaptiveBalanceManager, BalanceProfileConfig, BalanceUpdateEvent
from src.strategies.scalping.signal_generator import SignalGenerator
from src.strategies.scalping.order_executor import OrderExecutor
from src.strategies.scalping.position_manager import PositionManager
from src.strategies.scalping.performance_tracker import PerformanceTracker
...
class WebSocketScalpingOrchestrator:
    ...
    def _init_balance_manager(self):
        ...
        try:
            ...
            if self.config.balance_profiles:
                ...
                logger.info("✅ Adaptive Balance Manager initialized with config profiles")
            else:
                # Используем профили по умолчанию
                from src.balance import create_default_profiles
                profiles = create_default_profiles()
                self.balance_manager = AdaptiveBalanceManager(profiles)
                logger.info("✅ Adaptive Balance Manager initialized with default profiles")
    ...
    def _init_trading_modules(self):
        ...
        from src.strategies.modules.correlation_filter import CorrelationFilterConfig, CorrelationFilter
        ...
        from src.strategies.modules.volume_profile_filter import VolumeProfileConfig, VolumeProfileFilter
        ...
        self.correlation_filter = CorrelationFilter(self.okx_client, CorrelationFilterConfig(), self.config.trading.symbols)
        ...
        from src.strategies.scalping.signal_generator_config import SignalGeneratorConfig
        from src.strategies.scalping.order_executor_config import OrderExecutorConfig
        from src.strategies.scalping.position_manager_config import PositionManagerConfig
    ...
    async def start(self):
        ...
        try:
            ...
            if self.test_mode:
                logger.info("🧪 Test mode: WebSocket listener ready (not starting infinite loop)")
                return True
            else:
        ...
        while self.is_running:
            try:
                ...
                if not self.websocket_manager.is_connected:
                    logger.warning("⚠️ WebSocket disconnected, waiting for reconnect...")
    ...
    async def _process_symbol(self, symbol: str):
        ...
        try:
            ...
            try:
                ...
                # Обновление ARM
                self.arm.detect_regime(indicators_data.get("adx", 0.0), indicators_data.get("atr", 0.0))
                ...
                # Генерация сигнала
                signal = await self._generate_signal(symbol, current_price, indicators_data, regime_params)
    ...
    async def _generate_signal(self, symbol: str, price: float, indicators: Dict[str, Any], regime_params: Dict) -> Optional[Dict]:
        """Генерация торгового сигнала"""
        try:
            # Проверка cooldown
            current_time = time.time()
            if current_time - self.trading_state.last_signal_time < self.trading_state.signal_cooldown:
                return None
            ...
            if signal:
                ...
                logger.info(f"📊 Signal generated for {symbol}: {signal['side']} at {price}")
    ...
    async def _apply_filters(self, signal: Dict) -> bool:
        ...
        try:
            ...
            # Correlation фильтр
            if self.correlation_filter and not self.correlation_filter.check_entry(signal):
                return False
            ...
            # Pivot Points фильтр
            if self.pivot_filter and not self.pivot_filter.check_entry(symbol, side, price):
                return False
            
            # Volume Profile фильтр
            if self.volume_filter and not self.volume_filter.check_entry(symbol, side, price):
                return False
            
            # Balance Checker
            if self.balance_checker and not self.balance_checker.check_entry(symbol, side, price):
                return False
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            **self.stats,
            "websocket_status": self.websocket_manager.get_connection_status() if self.websocket_manager else {},
            "latency_stats": self.latency_monitor.get_latency_stats() if self.latency_monitor else {},
            "trading_state": {
