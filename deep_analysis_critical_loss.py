...
print(
    f"С нулевым duration: {len(zero_duration)} ({len(zero_duration)/len(critical)*100:.1f}%)"
)
...
for t in profitable_critical[:5]:
    print(
        f"{t.get('symbol')} {t.get('side')}: PnL=${float(t.get('net_pnl', 0)):.4f}, duration={float(t.get('duration_sec', 0)):.1f}s"
    )
