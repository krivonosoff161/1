...
from src.strategies.scalping.websocket_orchestrator import (
    WebSocketScalpingOrchestrator, WebSocketTradingState)
...
class TestWebSocketScalpingOrchestrator:
    ...
    async def test_start_success(self, mock_config, mock_okx_client):
        ...
        with patch.object(
            orchestrator.websocket_manager, "connect", new_callable=AsyncMock
        ) as mock_connect:
            with patch.object(
                orchestrator.websocket_manager,
                "subscribe_ticker",
                ...
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            orchestrator, "_trading_loop", new_callable=AsyncMock
                        ) as mock_trading:
    ...
    async def test_stop(self, mock_config, mock_okx_client):
        ...
        with patch.object(
            orchestrator.websocket_manager, "disconnect", new_callable=AsyncMock
        ) as mock_disconnect:
    ...
    async def test_process_symbol_stale_data(self, mock_config, mock_okx_client):
        ...
        orchestrator.trading_state.last_update_time[
            "ETH-USDT"
        ] = 0  # Очень старое время
