...
class AdaptiveBalanceManager:
    ...
    def _switch_profile(self, old_profile: Optional[BalanceProfile], 
        ...
        logger.info(f"🔄 Profile switched: {old_profile.value if old_profile else 'None'} → {new_profile.value}")
        logger.info(f"💰 Balance: ${old_balance:,.2f} → ${new_balance:,.2f} (${new_balance - old_balance:+,.2f})")
    ...
    def apply_to_regime_params(self, regime_params: Dict[str, Any], regime: str) -> Dict[str, Any]:
        ...
        # Логируем применение
        logger.debug(f"🎯 Applied {self.current_profile.value.upper()} profile to {regime} regime")
    ...
    def check_and_update_balance(self, event: str, **kwargs) -> bool:
        ...
        logger.debug(f"📊 Balance event: {event} | Symbol: {kwargs.get('symbol')} | Amount: {kwargs.get('amount')}")
