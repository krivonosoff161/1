...
class LogAnalyzer:
    ...
    def _extract_trade_from_line(self, line: str) -> Trade:
        ...
        try:
            ...
            price_match = re.search(
                r"entry[:\s]+(\d+\.?\d*).+exit[:\s]+(\d+\.?\d*)", line, re.IGNORECASE
            )
            if not price_match:
            ...
            pnl_percent = (
                float(pnl_percent_match.group(1)) if pnl_percent_match else 0.0
            )
    ...
    def calculate_metrics(self) -> TradingMetrics:
        ...
        for trade in self.trades:
            ...
            # По символам
            metrics.trades_by_symbol[trade.symbol] = (
                metrics.trades_by_symbol.get(trade.symbol, 0) + 1
            )
            metrics.pnl_by_symbol[trade.symbol] = (
                metrics.pnl_by_symbol.get(trade.symbol, 0.0) + trade.pnl
            )

            # По причинам закрытия
            metrics.trades_by_reason[trade.reason] = (
                metrics.trades_by_reason.get(trade.reason, 0) + 1
            )
    ...
    def identify_problems(self, metrics: TradingMetrics) -> List[str]:
        ...
        if metrics.win_rate < 40:
            problems.append(
                f"🔴 КРИТИЧНО: Очень низкий Win Rate ({metrics.win_rate:.1f}%). Проблемы с качеством сигналов!"
            )
        elif metrics.win_rate < 50:
            problems.append(
                f"⚠️  Низкий Win Rate ({metrics.win_rate:.1f}%). Нужно улучшить фильтрацию сигналов."
            )

        # Проблема 2: Плохое соотношение Win/Loss
        if metrics.avg_win_loss_ratio < 1.0:
            problems.append(
                f"🔴 КРИТИЧНО: Средний убыток больше средней прибыли (ratio: {metrics.avg_win_loss_ratio:.2f})!"
            )
        elif metrics.avg_win_loss_ratio < 1.5:
            problems.append(
                f"⚠️  Слабое соотношение прибыль/убыток ({metrics.avg_win_loss_ratio:.2f}). Рекомендуется > 1.5"
            )

        # Проблема 3: Низкий Profit Factor
        if metrics.profit_factor < 1.0:
            problems.append(
                f"🔴 КРИТИЧНО: Profit Factor < 1 ({metrics.profit_factor:.2f}). Стратегия убыточна!"
            )
        elif metrics.profit_factor < 1.5:
            problems.append(
                f"⚠️  Низкий Profit Factor ({metrics.profit_factor:.2f}). Цель > 1.5-2.0"
            )

        # Проблема 4: Много последовательных убытков
        if metrics.max_consecutive_losses > 5:
            problems.append(
                f"🔴 Слишком много последовательных убытков ({metrics.max_consecutive_losses}). Проверьте фильтры!"
            )

        # Проблема 5: Анализ причин закрытия
        if metrics.trades_by_reason.get("stop_loss", 0) > metrics.trades_by_reason.get(
            "take_profit", 0
        ):
            sl_count = metrics.trades_by_reason.get("stop_loss", 0)
            tp_count = metrics.trades_by_reason.get("take_profit", 0)
            problems.append(
                f"🔴 Stop-Loss срабатывает чаще ({sl_count}x), чем Take-Profit ({tp_count}x)!"
            )
    ...
    def generate_report(self):
        ...
        for reason, count in sorted(
            metrics.trades_by_reason.items(), key=lambda x: x[1], reverse=True
        ):
        ...
        if self.errors:
            ...
            for i, (error, count) in enumerate(
                sorted(error_counts.items(), key=lambda x: x[1], reverse=True)[:5], 1
            ):
    ...
    def _generate_recommendations(self, metrics: TradingMetrics, problems: List[str]):
        ...
        if metrics.win_rate < 50:
            recommendations.append(
                "1. Усилить фильтрацию сигналов (повысить требования к confidence)"
            )
        ...
        if metrics.avg_win_loss_ratio < 1.5:
            ...
            recommendations.append(
                "   - Уменьшить SL (например, с 0.45% до 0.35-0.40%)"
            )
        ...
        if metrics.profit_factor < 1.5:
            ...
            recommendations.append(
                "   - Протестировать на истории с новыми параметрами"
            )
