...
def comprehensive_system_analysis(log_file_path: str) -> Dict:
    ...
    # === ОБЩИЕ РЕКОМЕНДАЦИИ ===
    print(f'\n💡 ОБЩИЕ РЕКОМЕНДАЦИИ:')
    recommendations = generate_recommendations(regime_analysis, pair_analysis, system_analysis, adaptive_analysis, time_analysis)
...
def analyze_by_regime(signals: List[Dict], trades: List[Dict]) -> Dict:
    ...
    print(f'   📋 Режимы в сигналах: {collections.Counter(row.get("regime", "unknown") for row in signals)}')
    print(f'   📋 Режимы в сделках: {collections.Counter(row.get("regime", "unknown") for row in trades)}')
    ...
    for regime in set(row.get('regime', 'unknown') for row in signals + trades):
        ...
        if regime_trades:
            ...
            win_rate = len([t for t in regime_trades if float(t.get('net_pnl', 0)) > 0]) / len(regime_trades) * 100
            avg_duration = sum(float(t.get('duration_sec', 0)) for t in regime_trades) / len(regime_trades) / 60
        else:
            pnl = 0
            win_rate = 0
            avg_duration = 0
        ...
        print(f'   {regime.upper()}: {len(regime_signals)} сигн → {len(regime_trades)} сделок ({conversion:.1f}%) | P&L: {pnl:.2f} | WR: {win_rate:.1f}% | Avg: {avg_duration:.1f} мин')
...
def analyze_by_pairs(signals: List[Dict], trades: List[Dict]) -> Dict:
    ...
    for symbol in all_symbols:
        ...
        if pair_trades:
            ...
            win_rate = len([t for t in pair_trades if float(t.get('net_pnl', 0)) > 0]) / len(pair_trades) * 100
            reasons = collections.Counter(t.get('reason', 'unknown') for t in pair_trades)
        else:
            pnl = 0
            win_rate = 0
            reasons = {}
        ...
        print(f'   {symbol}: {len(pair_signals)} сигн → {len(pair_trades)} сделок ({conversion:.1f}%) | P&L: {pnl:.2f} | WR: {win_rate:.1f}%')
...
def analyze_systems(signals: List[Dict], trades: List[Dict]) -> Dict:
    ...
    big_profit_trades = [t for t in trades if 'profit_harvest' in t.get('reason', '') or 'max_holding' in t.get('reason', '')]
    print(f'   🤑 Big Profit система: {len(big_profit_trades)} сделок')

    # Анализ системы rebounds
    rebound_signals = [s for s in signals if 'rebound' in s.get('filters_passed', '').lower()]
    rebound_trades = [t for t in trades if any('rebound' in str(t.get(k, '')) for k in t.keys())]
    print(f'   🔄 Rebound система: {len(rebound_signals)} сигналов → {len(rebound_trades)} сделок')

    # Анализ time-based системы
    time_signals = [s for s in signals if 'time' in s.get('filters_passed', '').lower()]
    time_trades = [t for t in trades if float(t.get('duration_sec', 0)) > 1800]  # >30 мин
    print(f'   🕐 Time-based система: {len(time_signals)} сигналов → {len(time_trades)} сделок')
    ...
    return {
        ...
        'rebounds': {
            'signals': len(rebound_signals),
            'trades': len(rebound_trades)
        },
        'time_based': {
            'signals': len(time_signals),
            'trades': len(time_trades)
        }
    }
...
def analyze_adaptive_parameters(trades: List[Dict]) -> Dict:
    ...
    print(f'   🎯 TP/SL адаптивность: TP {tp_reasons}, SL {sl_reasons} ({sl_reasons/(tp_reasons+sl_reasons)*100:.1f}% SL)')

    # Анализ размеров позиций
    sizes = [float(t.get('size', 0)) for t in trades if t.get('size')]
    if sizes:
        print(f'   📏 Размеры позиций: min {min(sizes):.4f}, max {max(sizes):.4f}, avg {sum(sizes)/len(sizes):.4f}')

    # Анализ левериджа (если есть)
    leverages = [float(t.get('leverage', 0)) for t in trades if t.get('leverage')]
    if leverages:
        print(f'   ⚡ Леверидж: min {min(leverages)}, max {max(leverages)}, avg {sum(leverages)/len(leverages):.1f}')

    return {
        'tp_sl_ratio': sl_reasons / (tp_reasons + sl_reasons) if (tp_reasons + sl_reasons) > 0 else 0,
...
def generate_recommendations(regime_analysis, pair_analysis, system_analysis, adaptive_analysis, time_analysis) -> List[str]:
    """Генерация рекомендаций"""

    recommendations = []

    # Анализ режимов
    best_regime = max(regime_analysis.items(), key=lambda x: x[1]['pnl']) if regime_analysis else None
    worst_regime = min(regime_analysis.items(), key=lambda x: x[1]['pnl']) if regime_analysis else None

    if best_regime and worst_regime:
        recommendations.append(f"• Лучший режим: {best_regime[0]} (P&L: {best_regime[1]['pnl']:.2f})")
        recommendations.append(f"• Худший режим: {worst_regime[0]} (P&L: {worst_regime[1]['pnl']:.2f}) - требует настройки")

    # Анализ пар
    best_pair = max(pair_analysis.items(), key=lambda x: x[1]['pnl']) if pair_analysis else None
    worst_pair = min(pair_analysis.items(), key=lambda x: x[1]['pnl']) if pair_analysis else None

    if best_pair and worst_pair:
        recommendations.append(f"• Лучшая пара: {best_pair[0]} (P&L: {best_pair[1]['pnl']:.2f})")
        recommendations.append(f"• Худшая пара: {worst_pair[0]} (P&L: {worst_pair[1]['pnl']:.2f}) - адаптировать параметры")

    # Анализ систем
    if system_analysis['big_profit']['trades'] > 0:
        bp_pnl = system_analysis['big_profit']['pnl']
        recommendations.append(f"• Big Profit система: {bp_pnl:.2f} P&L - {'хорошая' if bp_pnl > 0 else 'нуждается в оптимизации'}")

    # Общие рекомендации
    if adaptive_analysis['tp_sl_ratio'] > 0.7:
        recommendations.append("• Критично: слишком много SL - оптимизировать TP/SL соотношение для каждого режима/пары")

    recommendations.append("• Необходимы индивидуальные параметры для каждой комбинации режим+пара")
