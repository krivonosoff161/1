...
class MathAuditor:
    ...
    def check_config_file(self, config_file: Path):
        ...
        try:
            ...
            if not any(issue["file"] == str(config_file.relative_to(self.codebase_path)) for issue in self.issues):
                print(f"  {GREEN}✅ config_futures.yaml: все режимы и символы присутствуют{RESET}")
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                if "long" in line.lower() and "pnl" in line.lower():
                    if "exit_price - entry_price" not in line and "current_price - entry_price" not in line:
                        if "entry_price - exit_price" in line or "entry_price - current_price" in line:
                            issues.append(f"Строка {i}: возможна ошибка знака для LONG PnL")
                            
                # Проверяем SHORT PnL
                if "short" in line.lower() and "pnl" in line.lower():
                    if "entry_price - exit_price" not in line and "entry_price - current_price" not in line:
                        if "exit_price - entry_price" in line or "current_price - entry_price" in line:
                            issues.append(f"Строка {i}: возможна ошибка знака для SHORT PnL")
    ...
    def print_summary(self):
        ...
        if not self.issues:
            ...
            print(f"{YELLOW}   Рекомендуется провести ручную проверку критических мест.{RESET}")
        else:
