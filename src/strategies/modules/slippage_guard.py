...
class SlippageGuard:
    ...
    def __init__(self, 
        ...
        logger.info(f"SlippageGuard инициализирован: max_slippage={max_slippage_percent:.3f}%, "
    ...
    async def _get_current_prices(self, client, symbol: str) -> Optional[Dict[str, float]]:
    ...
    def _should_cancel_order(self, 
        ...
        order_time = datetime.fromisoformat(order.get('cTime', '').replace('Z', '+00:00'))
        time_since_order = (datetime.now() - order_time).total_seconds()
        
        if time_since_order > self.order_timeout:
            logger.warning(f"Ордер {order.get('ordId')} превысил таймаут {self.order_timeout}с")
            return True
        
        # Проверка спреда
        if not slippage_analysis['is_spread_acceptable']:
            logger.warning(f"Спред слишком большой: {slippage_analysis['spread_percent']:.3f}%")
            return True
        
        # Проверка проскальзывания
        if not slippage_analysis['is_slippage_acceptable']:
            logger.warning(f"Проскальзывание слишком большое: {slippage_analysis['slippage_percent']:.3f}%")
            return True
    ...
    async def validate_order_before_placement(self, 
                                            symbol: str, 
                                            side: str, 
                                            order_type: str,
                                            price: Optional[float],
                                            size: float,
                                            client) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if order_type == 'limit' and price:
                ...
                if not slippage_analysis['is_slippage_acceptable']:
                    return False, f"Проскальзывание слишком большое: {slippage_analysis['slippage_percent']:.3f}%"
    ...
    def set_parameters(self, 
        ...
        logger.info(f"Параметры SlippageGuard обновлены: slippage={self.max_slippage_percent:.3f}%, "
