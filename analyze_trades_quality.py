...
def analyze_trades(csv_path: str):
    ...
    try:
        ...
        print(f"   ✅ Положительных: {positive_trades} ({positive_trades/total_trades*100:.1f}%)")
        print(f"   ❌ Отрицательных: {negative_trades} ({negative_trades/total_trades*100:.1f}%)")
        ...
        for symbol in symbol_stats.index:
            ...
            symbol_win_rate = (len(symbol_df[symbol_df["net_pnl"] > 0]) / len(symbol_df) * 100) if len(symbol_df) > 0 else 0
            print(f"   {symbol}:")
            print(f"      Сделок: {int(count)}")
        ...
        print("=" * 80)
        if win_rate < 50:
            print(f"   ❌ Win Rate ниже 50% ({win_rate:.1f}%) - больше убыточных сделок!")
        elif win_rate < 60:
        ...
        if total_pnl < 0:
            print(f"   ❌ Общий PnL отрицательный (${total_pnl:.4f} USDT) - бот в убытке!")
        elif total_pnl == 0:
        ...
        if len(profitable_df) > 0 and len(losing_df) > 0:
            if abs(avg_loss) > avg_profit:
                print(f"   ❌ Средний убыток (${abs(avg_loss):.4f}) больше средней прибыли (${avg_profit:.4f}) - нужно улучшить стоп-лоссы!")
            elif abs(avg_loss) == avg_profit:
                print(f"   ⚠️  Средний убыток равен средней прибыли - нужно улучшить risk/reward")
            else:
                print(f"   ✅ Средняя прибыль (${avg_profit:.4f}) больше среднего убытка (${abs(avg_loss):.4f})")
