...
class EnhancedScalpingStrategy:
    ...
    def __init__(
        ...
        logger.info(
            f"Enhanced scalping strategy initialized for {len(config.get('symbols', []))} symbols"
        )

    def _initialize_indicators(self):
        """Инициализация улучшенных индикаторов"""
        ...
        self.macd = AdaptiveMACD(
            fast_period=12, slow_period=26, signal_period=9, volatility_adjustment=True
        )
        ...
        self.regime_detector = MarketRegimeDetector(
            lookback_period=100, volatility_threshold=0.02, trend_threshold=0.001
        )

    def _initialize_filters(self):
        """Инициализация фильтров для улучшения качества сигналов"""
        ...
        # Фильтр корреляции
        self.max_correlation_exposure = 0.7  # Максимальная корреляция между позициями
    ...
    async def _close_position_market(self, symbol: str, reason: str):
