...
from src.config import BotConfig, RiskConfig, ScalpingConfig, TradingConfig
...
async def test_adaptive_parameters():
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        regime_params = orchestrator.arm.get_current_parameters(
            orchestrator.balance_manager
        )
        ...
        if case["expected_profile"] == "small":
            if case["regime"] == "trending":
                assert (
                    regime_params.tp_atr_multiplier > 2.0
                ), f"Small+Trending TP should be boosted: {regime_params.tp_atr_multiplier}"
                assert (
                    regime_params.sl_atr_multiplier < 1.5
                ), f"Small+Trending SL should be reduced: {regime_params.sl_atr_multiplier}"
            elif case["regime"] == "choppy":
                assert (
                    regime_params.tp_atr_multiplier < 2.0
                ), f"Small+Choppy TP should be reduced: {regime_params.tp_atr_multiplier}"
                assert (
                    regime_params.sl_atr_multiplier > 1.5
                ), f"Small+Choppy SL should be increased: {regime_params.sl_atr_multiplier}"
        elif case["expected_profile"] == "large":
            if case["regime"] == "trending":
                # Large+Trending должен быть более агрессивным чем базовый trending
                assert (
                    regime_params.tp_atr_multiplier >= 2.5
                ), f"Large+Trending TP should be at least 2.5: {regime_params.tp_atr_multiplier}"
                assert (
                    regime_params.sl_atr_multiplier <= 1.2
                ), f"Large+Trending SL should be at most 1.2: {regime_params.sl_atr_multiplier}"

        print(
            f"   ✅ Parameters correctly adapted for {case['expected_profile']} + {case['regime']}"
        )
