...
class FuturesPositionManager:
    ...
    async def _check_position_safety(self, position: Dict[str, Any]):
        ...
        try:
            ...
            is_safe, details = self.margin_calculator.is_position_safe(
                ...
                side, leverage, safety_threshold=1.5
            )
            
            if not is_safe:
                margin_ratio = details['margin_ratio']
                logger.warning(f"⚠️ Позиция {symbol} небезопасна: маржа {margin_ratio:.1f}%")
    ...
    def get_management_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            tp_rate = (self.management_stats['tp_closed'] / closed * 100) if closed > 0 else 0
            sl_rate = (self.management_stats['sl_closed'] / closed * 100) if closed > 0 else 0
            ...
            return {
                ...
                'last_position_time': self.position_history[-1]['close_time'] if self.position_history else None
            }
