...
from src.models import MarketData, OHLCV
...
class FuturesSignalGenerator:
    ...
    async def _generate_symbol_signals(self, symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Применение фильтров
            filtered_signals = await self._apply_filters(symbol, base_signals, market_data)
    ...
    async def _generate_base_signals(self, symbol: str, market_data: MarketData) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # RSI сигналы
            rsi_signals = await self._generate_rsi_signals(symbol, indicators, market_data)
            signals.extend(rsi_signals)
            
            # MACD сигналы
            macd_signals = await self._generate_macd_signals(symbol, indicators, market_data)
            signals.extend(macd_signals)
            
            # Bollinger Bands сигналы
            bb_signals = await self._generate_bollinger_signals(symbol, indicators, market_data)
            signals.extend(bb_signals)
            
            # Moving Average сигналы
            ma_signals = await self._generate_ma_signals(symbol, indicators, market_data)
    ...
    async def _generate_rsi_signals(self, symbol: str, indicators: Dict, market_data: MarketData) -> List[Dict[str, Any]]:
    ...
    async def _generate_macd_signals(self, symbol: str, indicators: Dict, market_data: MarketData) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if macd_line > signal_line and histogram > 0:
                signals.append({
                    ...
                    'side': 'buy',
                    'type': 'macd_bullish',
                    'strength': min(abs(histogram) / 100, 1.0),  # Нормализованная сила
                    'price': market_data.price,
                    'timestamp': market_data.timestamp,
    ...
    async def _generate_bollinger_signals(self, symbol: str, indicators: Dict, market_data: MarketData) -> List[Dict[str, Any]]:
    ...
    async def _generate_ma_signals(self, symbol: str, indicators: Dict, market_data: MarketData) -> List[Dict[str, Any]]:
    ...
    async def _apply_filters(self, symbol: str, signals: List[Dict[str, Any]], market_data: MarketData) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for signal in signals:
                ...
                # Проверка корреляции
                if not await self.correlation_filter.is_signal_valid(signal, market_data):
                    continue
    ...
    async def _adapt_signal_for_futures(self, signal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            futures_signal['leverage_adjusted_strength'] = signal['strength'] * (leverage / 3)
            
            # Добавление параметров маржи
            futures_signal['margin_required'] = True
            futures_signal['liquidation_risk'] = self._calculate_liquidation_risk(signal)
            
            # Адаптация размера позиции
            futures_signal['max_position_size'] = await self._calculate_max_position_size(signal)
    ...
    async def _filter_and_rank_signals(self, signals: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            filtered_signals = [s for s in signals if s.get('strength', 0) >= min_strength]
            
            # Ранжирование по силе и уверенности
            ranked_signals = sorted(
                filtered_signals,
                key=lambda x: (x.get('strength', 0) * x.get('confidence', 0), x.get('strength', 0)),
                reverse=True
            )
    ...
    def get_signal_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            sell_signals = sum(1 for r in self.signal_history if r.get('side') == 'sell')
