...
def analyze_position_timing(log_file_path: str) -> Dict:
    ...
    print(f'⏱️ Продолжительность: min={min(durations):.1f} мин, max={max(durations):.1f} мин, avg={sum(durations)/len(durations):.1f} мин')
    ...
    print(f'   ⚡ Быстрые (≤5 мин): {quick_closes} ({quick_closes/len(durations)*100:.1f}%)')
    print(f'   🕐 Средние (5-30 мин): {medium_closes} ({medium_closes/len(durations)*100:.1f}%)')
    print(f'   🕛 Долгие (>30 мин): {long_closes} ({long_closes/len(durations)*100:.1f}%)')
    ...
    if quick_closes > 0:
        ...
        if quick_closes / len(durations) > 0.5:
            print(f'   ⚠️ КРИТИЧНО: {quick_closes/len(durations)*100:.1f}% позиций закрываются за ≤5 минут!')
    ...
    if weekend_positions + weekday_positions > 0:
        ...
        print(f'   Будни: {weekday_positions} ({weekday_positions/(weekday_positions+weekend_positions)*100:.1f}%)')
        print(f'   Выходные: {weekend_positions} ({weekend_positions/(weekday_positions+weekend_positions)*100:.1f}%)')
    ...
    return {
        ...
        'weekend_ratio': weekend_positions/(weekend_positions+weekday_positions)*100 if weekend_positions+weekday_positions > 0 else 0
    }
