...
class LogAnalyzer:
    ...
    def analyze_trades(self):
        ...
        print(f"   Отрицательных duration: {negative_duration} ({negative_duration/total_trades*100:.1f}%)")
        print(f"   Нулевых duration: {zero_duration} ({zero_duration/total_trades*100:.1f}%)")
        
        if negative_duration > 0:
            print(f"\n   ⚠️ ПРИМЕРЫ ОТРИЦАТЕЛЬНЫХ DURATION:")
            negative_samples = df[df['duration_sec'] < 0].head(5)
            for _, row in negative_samples.iterrows():
                print(f"      {row['symbol']} {row['side']}: duration={row['duration_sec']:.2f}s, "
...
async def fetch_exchange_data(start_date: datetime, end_date: datetime):
    ...
    try:
        ...
        for pos in positions:
            print(f"   {pos.get('instId', 'N/A')}: {pos.get('pos', '0')} "
...
def main():
    archive_path = Path(r"C:\Users\krivo\simple trading bot okx\logs\futures\archived\logs_2025-12-06_15-58-40.zip")
