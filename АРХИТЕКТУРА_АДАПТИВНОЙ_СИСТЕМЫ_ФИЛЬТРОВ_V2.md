# 🏗️ Архитектура адаптивной системы фильтров V2 (упрощенная)

## ✅ Улучшения на основе критики Sonnet

### Проблемы V1:
1. ❌ Слишком сложные мультипликативные формулы (экспоненциальный рост)
2. ❌ Сложность отладки (11 × 6 = 66 комбинаций)
3. ❌ Риск переоптимизации и overfitting
4. ❌ Латентность вычислений

### Решения V2:
1. ✅ **Взвешенное суммирование** вместо мультипликативных цепочек
2. ✅ **Упрощенная система** (3-4 фактора вместо 6)
3. ✅ **Метрики и логирование** для валидации
4. ✅ **Оптимизация производительности** (агрессивное кэширование)

---

## 📊 Упрощенная матрица адаптации

### Принцип: Взвешенное суммирование

**Формула:**
```python
final_value = base * (1 + Σ(weight_i * factor_i))
```

**Преимущества:**
- Линейная зависимость (предсказуемо)
- Легче отлаживать
- Меньше риск overfitting
- Быстрее вычисляется

---

## 🎯 Упрощенная система (3 основных фактора)

### Фактор 1: Режим рынка (вес: 0.5)
- **trending**: +20%
- **ranging**: 0%
- **choppy**: -20%

### Фактор 2: Волатильность (вес: 0.3)
- **Высокая** (>3%): +30%
- **Средняя** (1-3%): 0%
- **Низкая** (<1%): -30%

### Фактор 3: Win Rate (вес: 0.2)
- **Высокий** (>60%): -20%
- **Средний** (40-60%): 0%
- **Низкий** (<40%): +20%

**Итого:** `1 + 0.5*regime + 0.3*vol + 0.2*winrate`

---

## 📋 Упрощенные формулы для всех параметров

### 1. V-образный разворот (reversal_threshold)

**База:** 0.15% (0.0015)

**Формула:**
```python
reversal_threshold = 0.0015 * (1 + 0.5*regime_factor + 0.3*vol_factor + 0.2*winrate_factor)
```

**Факторы:**
- `regime_factor`: trending=+0.2, ranging=0.0, choppy=-0.2
- `vol_factor`: high=+0.3, medium=0.0, low=-0.3
- `winrate_factor`: high=-0.2, medium=0.0, low=+0.2

**Примеры:**
- Trending + High Vol + Low Win Rate:
  - `0.0015 * (1 + 0.5*0.2 + 0.3*0.3 + 0.2*0.2) = 0.0015 * 1.23 = 0.18%` ✅
- Ranging + Low Vol + High Win Rate:
  - `0.0015 * (1 + 0.5*0.0 + 0.3*(-0.3) + 0.2*(-0.2)) = 0.0015 * 0.91 = 0.14%` ✅

**Ограничения:** min=0.05%, max=0.5%

---

### 2. ADX Threshold

**База:** 20.0

**Формула:**
```python
adx_threshold = 20.0 * (1 + 0.5*regime_factor + 0.3*vol_factor + 0.2*winrate_factor)
```

**Факторы:**
- `regime_factor`: trending=+0.15, ranging=-0.15, choppy=-0.25
- `vol_factor`: high=-0.2, medium=0.0, low=+0.2
- `winrate_factor`: high=-0.15, medium=0.0, low=+0.15

**Ограничения:** min=15.0, max=28.0

---

### 3. Correlation Threshold

**База:** 0.7

**Формула:**
```python
correlation_threshold = 0.7 * (1 + 0.5*regime_factor + 0.3*vol_factor + 0.2*winrate_factor)
```

**Факторы:**
- `regime_factor`: trending=-0.1, ranging=-0.15, choppy=-0.2
- `vol_factor`: high=0.0, medium=0.0, low=0.0 (не влияет)
- `winrate_factor`: high=-0.1, medium=0.0, low=+0.1

**Ограничения:** min=0.5, max=0.8

---

### 4. Min Signal Strength

**База:** 0.7

**Формула:**
```python
min_signal_strength = 0.7 * (1 + 0.5*regime_factor + 0.3*vol_factor + 0.2*winrate_factor)
```

**Факторы:**
- `regime_factor`: trending=-0.1, ranging=0.0, choppy=+0.1
- `vol_factor`: high=0.0, medium=0.0, low=0.0 (не влияет)
- `winrate_factor`: high=-0.15, medium=0.0, low=+0.15

**Ограничения:** min=0.5, max=0.85

---

### 5. MACD Strength Divisor

**База:** 200.0

**Формула:**
```python
macd_divisor = 200.0 * (1 + 0.5*regime_factor + 0.3*vol_factor + 0.2*winrate_factor)
```

**Факторы:**
- `regime_factor`: trending=-0.15, ranging=+0.15, choppy=+0.25
- `vol_factor`: high=-0.25, medium=0.0, low=+0.25
- `winrate_factor`: high=0.0, medium=0.0, low=0.0 (не влияет)

**Ограничения:** min=150.0, max=280.0

---

### 6. Strength Multiplier

**База:** 2000.0

**Формула:**
```python
strength_multiplier = 2000.0 * (1 + 0.5*regime_factor + 0.3*vol_factor)
```

**Факторы:**
- `regime_factor`: trending=+0.15, ranging=-0.1, choppy=-0.15
- `vol_factor`: high=+0.25, medium=0.0, low=-0.15

**Ограничения:** min=1500.0, max=2800.0

---

### 7-11. Остальные параметры (аналогично)

Все используют ту же формулу с разными факторами и весами.

---

## 🏛️ Упрощенная архитектура

### Класс: `AdaptiveFilterParametersV2`

```python
class AdaptiveFilterParametersV2:
    """
    Упрощенная адаптивная система фильтров.
    Взвешенное суммирование вместо мультипликативных цепочек.
    """
    
    # Веса факторов (глобальные)
    WEIGHTS = {
        "regime": 0.5,      # 50% влияния
        "volatility": 0.3,  # 30% влияния
        "winrate": 0.2,     # 20% влияния
    }
    
    # Факторы по режимам
    REGIME_FACTORS = {
        "reversal_threshold": {
            "trending": 0.2,
            "ranging": 0.0,
            "choppy": -0.2,
        },
        "adx_threshold": {
            "trending": 0.15,
            "ranging": -0.15,
            "choppy": -0.25,
        },
        # ... для всех параметров
    }
    
    # Факторы по волатильности
    VOLATILITY_FACTORS = {
        "reversal_threshold": {
            "high": 0.3,    # >3%
            "medium": 0.0,  # 1-3%
            "low": -0.3,    # <1%
        },
        # ... для всех параметров
    }
    
    # Факторы по win rate
    WINRATE_FACTORS = {
        "reversal_threshold": {
            "high": -0.2,   # >60%
            "medium": 0.0,  # 40-60%
            "low": 0.2,     # <40%
        },
        # ... для всех параметров
    }
    
    def __init__(
        self,
        regime_manager: AdaptiveRegimeManager,
        data_registry: DataRegistry,
        trading_statistics: TradingStatistics,
    ):
        self.regime_manager = regime_manager
        self.data_registry = data_registry
        self.trading_statistics = trading_statistics
        
        # Агрессивное кэширование (60 секунд)
        self._cache: Dict[str, Tuple[float, float]] = {}  # {key: (value, timestamp)}
        self._cache_ttl = 60.0
        
        # Метрики для валидации
        self.metrics = {
            "parameter_changes": [],  # История изменений
            "regime_switches": [],    # История смен режимов
            "correlations": {},      # Корреляции параметров с PnL
        }
    
    def get_reversal_threshold(
        self, symbol: str, regime: str = None
    ) -> float:
        """Адаптивный порог V-образного разворота"""
        cache_key = f"reversal_threshold_{symbol}_{regime}"
        
        # Проверяем кэш
        if cache_key in self._cache:
            value, timestamp = self._cache[cache_key]
            if time.time() - timestamp < self._cache_ttl:
                return value
        
        # Базовое значение
        base = 0.0015
        
        # Получаем факторы
        regime_factor = self._get_regime_factor("reversal_threshold", regime, symbol)
        vol_factor = self._get_volatility_factor("reversal_threshold", symbol)
        winrate_factor = self._get_winrate_factor("reversal_threshold", symbol)
        
        # Взвешенное суммирование
        multiplier = 1.0 + (
            self.WEIGHTS["regime"] * regime_factor +
            self.WEIGHTS["volatility"] * vol_factor +
            self.WEIGHTS["winrate"] * winrate_factor
        )
        
        threshold = base * multiplier
        
        # Ограничения
        threshold = max(0.0005, min(0.005, threshold))
        
        # Кэшируем
        self._cache[cache_key] = (threshold, time.time())
        
        # Логируем метрики
        self._log_parameter_change("reversal_threshold", symbol, base, threshold, {
            "regime_factor": regime_factor,
            "vol_factor": vol_factor,
            "winrate_factor": winrate_factor,
        })
        
        return threshold
    
    def _get_regime_factor(self, param_name: str, regime: str, symbol: str) -> float:
        """Получить фактор режима"""
        if not regime:
            regime = self.regime_manager.get_current_regime(symbol).value.lower()
        
        factors = self.REGIME_FACTORS.get(param_name, {})
        return factors.get(regime, 0.0)
    
    def _get_volatility_factor(self, param_name: str, symbol: str) -> float:
        """Получить фактор волатильности"""
        # Получаем ATR
        atr = self.data_registry.get_atr(symbol, period=14)
        current_price = self.data_registry.get_current_price(symbol)
        
        if not atr or not current_price:
            return 0.0
        
        volatility_pct = (atr / current_price) * 100
        
        # Определяем уровень волатильности
        if volatility_pct > 3.0:
            vol_level = "high"
        elif volatility_pct < 1.0:
            vol_level = "low"
        else:
            vol_level = "medium"
        
        factors = self.VOLATILITY_FACTORS.get(param_name, {})
        return factors.get(vol_level, 0.0)
    
    def _get_winrate_factor(self, param_name: str, symbol: str) -> float:
        """Получить фактор win rate"""
        stats = self.trading_statistics.get_symbol_stats(symbol, lookback_hours=24)
        win_rate = stats.get("win_rate", 0.5)
        
        # Определяем уровень win rate
        if win_rate > 0.6:
            wr_level = "high"
        elif win_rate < 0.4:
            wr_level = "low"
        else:
            wr_level = "medium"
        
        factors = self.WINRATE_FACTORS.get(param_name, {})
        return factors.get(wr_level, 0.0)
    
    def _log_parameter_change(
        self, param_name: str, symbol: str, base: float, final: float, factors: Dict
    ):
        """Логировать изменение параметра для валидации"""
        change = {
            "timestamp": time.time(),
            "param_name": param_name,
            "symbol": symbol,
            "base": base,
            "final": final,
            "change_pct": ((final - base) / base) * 100,
            "factors": factors,
        }
        
        self.metrics["parameter_changes"].append(change)
        
        # Храним только последние 1000 изменений
        if len(self.metrics["parameter_changes"]) > 1000:
            self.metrics["parameter_changes"] = self.metrics["parameter_changes"][-1000:]
        
        # Логируем только значительные изменения (>10%)
        if abs(change["change_pct"]) > 10:
            logger.debug(
                f"📊 {param_name} для {symbol}: {base:.4f} → {final:.4f} "
                f"({change['change_pct']:+.1f}%) | "
                f"regime={factors.get('regime_factor', 0):.2f}, "
                f"vol={factors.get('vol_factor', 0):.2f}, "
                f"wr={factors.get('winrate_factor', 0):.2f}"
            )
```

---

## 📊 Система метрик для валидации

### Метрика 1: Частота смены режима

```python
def get_regime_stability(self, symbol: str, minutes: int = 60) -> float:
    """Проверить стабильность режима"""
    switches = [
        s for s in self.metrics["regime_switches"]
        if s["symbol"] == symbol and s["timestamp"] > time.time() - minutes * 60
    ]
    
    # Если режим меняется чаще чем раз в 15 минут - это шум
    if len(switches) > minutes / 15:
        logger.warning(
            f"⚠️ {symbol}: Режим меняется слишком часто ({len(switches)} раз за {minutes} мин) - возможен шум"
        )
        return 0.0  # Нестабильно
    
    return 1.0 - (len(switches) / (minutes / 15))  # 0.0 = нестабильно, 1.0 = стабильно
```

### Метрика 2: Корреляция параметров с PnL

```python
def calculate_parameter_correlation(self, param_name: str, symbol: str):
    """Рассчитать корреляцию между параметром и PnL"""
    changes = [
        c for c in self.metrics["parameter_changes"]
        if c["param_name"] == param_name and c["symbol"] == symbol
    ]
    
    # Получаем PnL за те же периоды
    pnl_changes = []
    for change in changes:
        stats = self.trading_statistics.get_symbol_stats(
            symbol, 
            start_time=change["timestamp"] - 300,  # 5 минут до
            end_time=change["timestamp"] + 300     # 5 минут после
        )
        pnl_changes.append(stats.get("pnl", 0.0))
    
    # Рассчитываем корреляцию
    if len(changes) > 10:
        correlation = np.corrcoef(
            [c["change_pct"] for c in changes],
            pnl_changes
        )[0, 1]
        
        self.metrics["correlations"][f"{param_name}_{symbol}"] = correlation
        
        logger.info(
            f"📈 Корреляция {param_name} с PnL для {symbol}: {correlation:.3f}"
        )
```

### Метрика 3: Стабильность параметров

```python
def get_parameter_stability(self, param_name: str, symbol: str) -> float:
    """Проверить стабильность параметра"""
    changes = [
        c for c in self.metrics["parameter_changes"]
        if c["param_name"] == param_name and c["symbol"] == symbol
        and c["timestamp"] > time.time() - 3600  # Последний час
    ]
    
    if len(changes) < 2:
        return 1.0  # Нет данных
    
    # Стандартное отклонение изменений
    change_pcts = [c["change_pct"] for c in changes]
    std = np.std(change_pcts)
    
    # Если std > 20% - параметр нестабилен
    stability = max(0.0, 1.0 - (std / 20.0))
    
    return stability
```

---

## ⚡ Оптимизация производительности

### 1. Агрессивное кэширование (60 секунд)

```python
# Кэш для всех параметров
self._cache: Dict[str, Tuple[float, float]] = {}
self._cache_ttl = 60.0  # 60 секунд
```

### 2. Batch вычисления

```python
def get_all_parameters(self, symbol: str, regime: str = None) -> Dict[str, float]:
    """Получить все параметры за один вызов (оптимизация)"""
    # Вычисляем все факторы один раз
    regime_factors = self._get_all_regime_factors(regime, symbol)
    vol_factors = self._get_all_volatility_factors(symbol)
    winrate_factors = self._get_all_winrate_factors(symbol)
    
    # Применяем ко всем параметрам
    params = {}
    for param_name in self.BASE_VALUES:
        base = self.BASE_VALUES[param_name]
        multiplier = 1.0 + (
            self.WEIGHTS["regime"] * regime_factors[param_name] +
            self.WEIGHTS["volatility"] * vol_factors[param_name] +
            self.WEIGHTS["winrate"] * winrate_factors[param_name]
        )
        params[param_name] = base * multiplier
    
    return params
```

### 3. Ленивая загрузка

```python
# Вычисляем только при необходимости
@lru_cache(maxsize=100)
def _get_volatility_level(self, symbol: str) -> str:
    """Ленивая загрузка уровня волатильности"""
    # ... вычисление
```

---

## 📝 Конфигурация (упрощенная)

```yaml
adaptive_filters_v2:
  enabled: true
  
  # Веса факторов (глобальные)
  weights:
    regime: 0.5      # 50%
    volatility: 0.3  # 30%
    winrate: 0.2     # 20%
  
  # Базовые значения
  base_values:
    reversal_threshold: 0.0015  # 0.15%
    adx_threshold: 20.0
    correlation_threshold: 0.7
    min_signal_strength: 0.7
    macd_divisor: 200.0
    strength_multiplier: 2000.0
  
  # Факторы по режимам
  regime_factors:
    reversal_threshold:
      trending: 0.2
      ranging: 0.0
      choppy: -0.2
    # ... для всех параметров
  
  # Факторы по волатильности
  volatility_factors:
    reversal_threshold:
      high: 0.3
      medium: 0.0
      low: -0.3
    # ... для всех параметров
  
  # Факторы по win rate
  winrate_factors:
    reversal_threshold:
      high: -0.2
      medium: 0.0
      low: 0.2
    # ... для всех параметров
  
  # Ограничения
  limits:
    reversal_threshold:
      min: 0.0005
      max: 0.005
    # ... для всех параметров
  
  # Кэширование
  cache_ttl_seconds: 60
  
  # Метрики
  metrics:
    enabled: true
    log_threshold_pct: 10.0  # Логировать изменения >10%
    correlation_window_hours: 24
```

---

## ✅ Преимущества V2

1. **Простота** - взвешенное суммирование вместо мультипликативных цепочек
2. **Предсказуемость** - линейная зависимость
3. **Отладка** - меньше факторов, проще понять причину
4. **Производительность** - агрессивное кэширование, batch вычисления
5. **Валидация** - метрики для проверки эффективности
6. **Безопасность** - min/max ограничения

---

## 🚀 План внедрения

1. **Этап 1:** Создать `AdaptiveFilterParametersV2` с упрощенными формулами
2. **Этап 2:** Добавить систему метрик
3. **Этап 3:** Интегрировать в `SignalGenerator`
4. **Этап 4:** Тестирование на исторических данных
5. **Этап 5:** Мониторинг метрик на живом рынке
6. **Этап 6:** Калибровка на основе метрик

---

**Дата:** 2025-12-23  
**Статус:** Упрощенная архитектура готова к реализации

