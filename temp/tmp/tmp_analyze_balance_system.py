...
for balance in test_balances:
    ...
    print(
        f"   Номинальный объем: ${total_notional:.2f} ({notional_percent:.1f}% баланса)"
    )
    print(f"   Запас: ${balance - total_margin:.2f} ({100 - usage_percent:.1f}%)")

    if usage_percent > 50:
        print(
            f"   ⚠️ ПРОБЛЕМА: Используется {usage_percent:.1f}% баланса - слишком много!"
        )
    elif usage_percent < 10:
        print(
            f"   ⚠️ ПРОБЛЕМА: Используется только {usage_percent:.1f}% баланса - слишком мало!"
        )
...
for pos_size in position_sizes:
    ...
    print(
        f"${pos_size:<19} {percent:>6.2f}%        {recommended:>6.2f}% (рекомендуется)"
    )
...
for balance in [200, 500, 800, 1018, 1200, 1500, 2000, 3000]:
    ...
    print(
        f"   Баланс ${balance:>6}: профиль {profile_name:>6}, размер ${size:>6.2f}, использование {usage:>5.1f}%"
    )
