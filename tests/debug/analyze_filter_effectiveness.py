...
def analyze_filter_effectiveness(log_file_path: str) -> Dict:
    ...
    print(
        f"🎚️ Confidence: min={min(confidences):.1f}, max={max(confidences):.1f}, avg={sum(confidences)/len(confidences):.1f}"
    )
    print(
        f"💪 Strength: min={min(strengths):.1f}, max={max(strengths):.1f}, avg={sum(strengths)/len(strengths):.1f}"
    )
    ...
    if filtered_signals:
        filter_reasons = collections.Counter(
            row.get("filter_reason", "unknown") for row in filtered_signals
        )
    ...
    if len(passed_signals) / total_signals < 0.2:
        print(
            f"   ⚠️ КРИТИЧНО: Конверсия слишком низкая! Фильтры отсеивают {100-len(passed_signals)/total_signals*100:.1f}% сигналов"
        )
    ...
    for symbol in symbols:
        ...
        print(
            f"   📊 {symbol}: {len(symbol_passed)}/{len(symbol_signals)} ({conversion:.1f}%)"
        )
        ...
        "symbols_analysis": {
            symbol: len(
                [
                    s
                    for s in signals
                    if s["symbol"] == symbol and s.get("filtered") != "true"
                ]
            )
            / count
            * 100
